\part{2017}
\renewcommand\resultsyear{2017}

\chapter{Referendums in 2017}

%There were no referendums in 2017.

\section{Burnley mayoral referendum}
\index{Burnley!Mayoral Referendum}

A referendum was held in Burnley on 4th May on the question of whether the district should have a directly elected mayor.

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.545\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
& Yes & 8694\\
& No & 10986\\
\end{tabular*}

%\part{By-elections}

\chapter{Parliamentary by-elections}

There were two parliamentary by-elections in 2017.

CommLg = Communist League

CPA = Christian Peoples Alliance

\section*{Copeland \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[23rd February; C gain from Lab]}

\index{Copeland , House of Commons@Copeland, \emph{House of Commons}}

Resignation of Jamie Reed (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Trudy Harrison & C & 13748\\
Gillian Troughton & Lab & 11601\\
Rebecca Hanson & LD & 2252\\
Fiona Mills & UKIP & 2025\\
Michael Guest & Ind & 811\\
Jack Lenox & Grn & 515\\
Roy Ivinson & Ind & 116\\
\end{tabular*}

\section*{Stoke-on-Trent Central \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[23rd February]}

\index{Stoke-on-Trent Central , House of Commons@Stoke-on-Trent C., \emph{House of Commons}}

Resignation of Tristram Hunt (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Gareth Snell & Lab & 7853\\
Paul Nuttall & UKIP & 5233\\
Jack Brereton & C & 5154\\
Zulfiqar Ali & LD & 2083\\
Adam Colclough & Grn & 294\\
Barbara Fielding-Morriss & Ind & 137\\
The Incredible Flying Brick & Loony & 127\\
David Furness & BNP & 124\\
Godfrey Davies & CPA & 109\\
Mohammed Akram & Ind & 56\\
\end{tabular*}

At the dissolution of Parliament on 3rd May 2017 there was an unfilled vacancy in Manchester Gorton due to the death of Sir Gerald Kaufman (Lab).  A by-election to fill this seat had been scheduled for 4th May but was superseded by the dissolution.
\index{Manchester Gorton , House of Commons@Manchester Gorton, \emph{House of Commons}}

\chapter{By-elections to devolved assemblies, the European Parliament, and police and crime commissionerships}

\section{Greater London Authority}

There were no by-elections in 2017 to the Greater London Authority.

Kemi Badenoch (C, London-wide list) resigned in June 2017.  She was replaced from the list by Susan Hall.

\section{National Assembly for Wales}

There were no by-elections in 2017 to the National Assembly for Wales.

Nathan Gill (Ind elected as UKIP) resigned on 27 December 2017.  He was replaced from the UKIP party list by Mandy Jones.

\section{Scottish Parliament}

There was one by-election in 2017 to the Scottish Parliament.

\subsection*{Ettrick, Roxburgh and Berwickshire \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Ettrick, Roxburgh and Berwickshire , Scottish Parliament@Ettrick, Roxburgh \& Berwickshire, \emph{Scot. Parl.}}

Resignation of John Lamont (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Rachael Hamilton & C & 20658\\
Gail Hendry & SNP & 11320\\
Sally Prentice & Lab & 3406\\
Catriona Bhatia & LD & 3196\\
\end{tabular*}

Rachael Hamilton (C, South of Scotland) resigned on 3 May 2017 in order to contest the above by-election.  She was replaced from the list by Michelle Ballantyne.

Douglas Ross MP (C, Highlands and Islands) resigned on 9 June 2017.  He was replaced from the list by Jamie Halcro Johnston.

Ross Thomson MP (C, North East) resigned on 9 June 2017.  He was replaced from the list by Tom Mason.

\section{Northern Ireland Assembly}

Vacancies in the Northern Ireland Assembly are filled by co-option.
%No co-options were made in 2017.
%
The following members were co-opted to the Assembly in 2017:
\begin{itemize}
\item Colm Gildernew (SF) replaced Michelle Gildernew MP following her resignation on 8th June (Fermanagh and South Tyrone).
\item Trevor Clarke (DUP) replaced Paul Girvan MP following his resignation on 8th June (South Antrim).
\item Emma Rogan (SF) replaced Chris Hazzard MP following his resignation on 8th June (South Down).
\item Karen Mullan (SF) replaced Elisha McCallion MP following her resignation on 8th June (Foyle).
\item Catherine Kelly (SF) replaced Barry McElduff MP following his resignation on 8th June (West Tyrone).
\end{itemize}

\section{European Parliament}

UK vacancies in the European Parliament are filled by the next available person from the party list at the most recent election (which was held in 2014).
%No replacements were made in 2017.
The following replacements were made in 2017:
\begin{itemize}
\item John Howarth (Lab) replaced Anneliese Dodds following her resignation on 8th June (South East).
\item John Flack (C) replaced Vicky Ford following her resignation on 8th June (Eastern).
\item Wajid Khan (Lab) replaced Afzal Khan following his resignation on 8th June (North West).
\item Rupert Matthews (C) replaced Andrew Lewer following his resignation on 8th June (East Midlands).
\item Baroness Mobarik (C) replaced Ian Duncan following his resignation on 22nd June (Scotland).
\item Jonathan Bullock (UKIP) replaced Roger Helmer following his resignation on 28th July (East Midlands).
\item Rory Palmer (Lab) replaced Dame Glenis Willmott following her resignation on 2nd October (East Midlands).
\end{itemize}

\section{Police and crime commissioners}

There were no by-elections in 2017 for vacant police and crime commissioner posts.

\chapter{Local by-elections and unfilled vacancies}

\begin{resultsiii}

\section{North London}

\council{City of London}

At the March 2017 ordinary election there were unfilled vacancies in Bishopsgate and Queenhithe wards due to the resignation of Billy Dove and the election of Alastair King as alderman (both Ind) respectively.
\index{Bishopsgate , City of London@Bishopsgate, \emph{City of London}}
\index{Queenhithe , City of London@Queenhithe, \emph{City of London}}

\subsubsection*{Farringdon Without \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[Wednesday 9th February]}

\index{Farringdon Without , City of London@Farringdon Wt., \emph{City of London}}

Aldermanic election: resignation of Julian Malins (Ind).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Greg Jones & Ind & 443\\
Robert Hughes-Penney & Ind & 98\\
Helen Carr & Ind & 26\\
\end{tabular*}

\subsubsection*{Castle Baynard \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[Wednesday 22nd March]}

\index{Castle Baynard , City of London@Castle Baynard, \emph{City of London}}

Aldermanic election: Ian Luder (Ind) sought re-election.

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Ian Luder & Ind & \emph{unop.}\\
\end{tabular*}

\subsubsection*{Aldersgate \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[Wednesday 3rd May]}

\index{Aldersgate , City of London@Aldersgate, \emph{City of London}}

Aldermanic election: Nicholas Anstee (Ind) sought re-election.

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Nicholas Anstee & Ind & 367\\
Tanvir Tayab & Ind & 25\\
\end{tabular*}

\subsubsection*{Langbourn \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[Monday 11th September]}

\index{Langbourn , City of London@Langbourn, \emph{City of London}}

Aldermanic election: Sir David Wootton (Ind) sought re-election.

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
David Wootton & Ind & \emph{unop.}\\
\end{tabular*}

\subsubsection*{Tower \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[Tuesday 12th September]}

\index{Tower , City of London@Tower, \emph{City of London}}

Aldermanic election: death of Sir Paul Judge (Ind).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Nicholas Lyons & Ind & 91\\
Marianne Fredericks & Ind & 61\\
James Tumbridge & Ind & 52\\
Andrew Marsden & Ind & 37\\
Jonathan Hindle & Ind & 6\\
Jonathan Pettigrew & Ind & 3\\
\end{tabular*}

\subsubsection*{Bishopsgate \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[Tuesday 14th November]}

\index{Bishopsgate , City of London@Bishopsgate, \emph{City of London}}

Resignation of Pooja Suri Tank (Ind).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Benjamin Murphy & Ind & 112\\
Patrick Streeter & Ind & 35\\
Timothy Becker & Ind & 18\\
\end{tabular*}

\subsubsection*{Portsoken \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[14th December]}

\index{Portsoken , City of London@Portsoken, \emph{City of London}}

Aldermanic election: resignation of Michael Bear (Ind).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.6\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Prem Goyal & Ind & 228\\
William Campbell-Taylor & Lab & 143\\
David Barker & Ind & 20\\
Anthony Samuels & Ind & 2\\
\end{tabular*}

\council{Camden}

\subsubsection*{Gospel Oak \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Gospel Oak , Camden@Gospel Oak, \emph{Camden}}

Resignation of Maeve McCormack (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Marcus Boyland & Lab & 1468\\
Stephen Crosher & LD & 587\\
Marx de Morais & C & 523\\
Dee Searle & Grn & 273\\
Giles Game & UKIP & 75\\
\end{tabular*}

\subsubsection*{Gospel Oak \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[9th November]}

\index{Gospel Oak , Camden@Gospel Oak, \emph{Camden}}

Resignation of Theo Blackwell (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l >{\tabularnums}r @{\extracolsep{\fill}}}
Jenny Mulholland & Lab & 1144\\
Jill Fraser & LD & 510\\
Marx de Morais & C & 303\\
Max Spencer & EDP & 31\\
\end{tabular*}

\council{Enfield}

\subsubsection*{Enfield Lock \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[18th May]}

\index{Enfield Lock , Enfield@Enfield Lock, \emph{Enfield}}

Resignation of Ozzie Uzoanya (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.55\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Elif Erbil & Lab & 2155\\
Christine Bellas & C & 973\\
Kate McGeevor & Grn & 104\\
Gary Robbens & UKIP & 91\\
Richard Morgan-Ash & LD & 54\\
\end{tabular*}

\subsubsection*{Enfield Highway \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[7th December]}

\index{Enfield Highway , Enfield@Enfield Highway, \emph{Enfield}}

Death of Turgut Esendagli (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.55\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Ergun Eren & Lab & 1619\\
Andrew Thorp & C & 620\\
Andreea Malin & Grn & 79\\
\end{tabular*}

\council{Hammersmith and Fulham}

\subsubsection*{Avonmore and Brook Green \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Avonmore and Brook Green , Hammersmith and Fulham@Avonmore \& Brook Green, \emph{Hammersmith \& Fulham}}

Resignation of Hannah Barlow (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.6\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
David Morton & Lab & 2795\\
William Marshall & C & 1849\\
Irina von Wiese und Kaiserwaldau & LD & 653\\
\end{tabular*}

\subsubsection*{Sands End \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Sands End , Hammersmith and Fulham@Sands End, \emph{Hammersmith \& Fulham}}

Resignation of Robert Largan (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Jackie Borland & C & 2845\\
Anne Rosenberg & Lab & 2669\\
Raymond Burnet & LD & 602\\
\end{tabular*}

\council{Harrow}

\subsubsection*{Roxbourne \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[9th March]}

\index{Roxbourne , Harrow@Roxbourne, \emph{Harrow}}

Death of Bob Currie (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Maxine Henson & Lab & 1554\\
Annabel Singh & C & 533\\
Marshel Amutharasan & LD & 240\\
Herbert Crossman & UKIP & 148\\
\end{tabular*}

\subsubsection*{Kenton East \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[20th April; C gain from Lab]}

\index{Kenton East , Harrow@Kenton E., \emph{Harrow}}

Death of Mitzi Green (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Nitesh Hirani & C & 1585\\
Nish Patel & Lab & 1328\\
Charis Croft & LD & 65\\
Herbert Crossman & UKIP & 54\\
\end{tabular*}

\council{Waltham Forest}

\subsubsection*{William Morris \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[29th June]}

\index{William Morris , Waltham Forest@William Morris, \emph{Waltham Forest}}

Death of Nadeem Ali (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Umar Ali & Lab & 1923\\
Rebecca Tully & Grn & 524\\
Afzal Akram & C & 365\\
\end{tabular*}

\columnbreak

\section{South London}

\council{Croydon}

\subsubsection*{South Norwood \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[7th September]}

\index{South Norwood , Croydon@South Norwood, \emph{Croydon}}

Resignation of Kathy Bee (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Patsy Cummings & Lab & 1671\\
Rebecca Natrajan & C & 475\\
Claire Bonham & LD & 388\\
Peter Underwood & Grn & 218\\
Michael Swadling & UKIP & 78\\
\end{tabular*}

\council{Merton}

\subsubsection*{St Helier \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[20th July]}

\index{Saint Helier , Merton@St Helier, \emph{Merton}}

Resignation of Imran Uddin (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Kelly Braund & Lab & 1508\\
Geraldine Kirby & C & 318\\
Geoff Cooper & LD & 98\\
Phillipa Maslin & Grn & 61\\
Bob Grahame & UKIP & 50\\
\end{tabular*}

\council{Wandsworth}

\subsubsection*{Thamesfield \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[9th November]}

\index{Thamesfield , Wandsworth@Thamesfield, \emph{Wandsworth}}

Death of Jim Maddan (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
John Locker & C & 1910\\
Sally Warren & Lab & 1101\\
Ryan Mercer & LD & 619\\
Di McCann & Grn & 275\\
\end{tabular*}

\section{Greater Manchester}

\council{Bury}

\subsubsection*{Besses \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Besses , Bury@Besses, \emph{Bury}}

Resignation of Alan Matthews (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Andrea Simpson & Lab & 1371\\
Satyen Sinha & C & 682\\
Gavin Warnes & LD & 415\\
Stephen Morris & EDP & 188\\
Janneke Calle & Grn & 86\\
\end{tabular*}

\subsubsection*{Radcliffe East \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Radcliffe East , Bury@Radcliffe E., \emph{Bury}}

Resignation of Nick Parnell (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Karen Leach & Lab & 2861\\
James Mason & C & 2046\\
Robert Graham & LD & 232\\
Nicole Haydock & Grn & 221\\
\end{tabular*}

\council{Manchester}

\subsubsection*{Rusholme \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Rusholme , Manchester@Rusholme, \emph{Manchester}}

Resignation of Kate Chappell (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Jill Lovecy & Lab & 2188\\
Amaan Hashmi & LD & 576\\
Anne Tucker & Grn & 458\\
David Semple & C & 151\\
Jack Metcalf & TUSC & 37\\
\end{tabular*}

\subsubsection*{Fallowfield \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[27th July]}

\index{Fallowfield , Manchester@Fallowfield, \emph{Manchester}}

Resignation of Mike Amesbury MP (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Ali Ilyas & Lab & 861\\
Adam King & Grn & 105\\
Alex Warren & LD & 82\\
David Semple & C & 72\\
\end{tabular*}

\council{Oldham}

\subsubsection*{Failsworth East \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[16th February]}

\index{Failsworth East , Oldham@Failsworth E., \emph{Oldham}}

Resignation of Jim McMahon MP (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Paul Jacques & Lab & 829\\
Antony Cahill & C & 360\\
Nicholas Godleman & UKIP & 166\\
Andy Hunter-Rossall & Grn & 49\\
Shaun Duffy & LD & 16\\
\end{tabular*}

\subsubsection*{Royton North \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Royton North , Oldham@Royton N., \emph{Oldham}}

Death of Tony Larkin (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Clint Phythian & Lab & 2504\\
Alan Fish & C & 2047\\
Russ Gosling & LD & 294\\
Lina Shaw & Grn & 205\\
\end{tabular*}

\council{Salford}

Republic = Republic Party

\subsubsection*{Kersal \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[2nd March; C gain from Lab]}

\index{Kersal , Salford@Kersal, \emph{Salford}}

Death of Harry Davies (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Arnie Saunders & C & 850\\
Mike Pevitt & Lab & 553\\
Jonny Wineberg & Ind & 354\\
Christopher Barnes & UKIP & 182\\
Jason Reading & Grn & 48\\
Adam Slack & LD & 39\\
\end{tabular*}

\subsubsection*{Claremont \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Claremont , Salford@Claremont, \emph{Salford}}

Resignation of Sareda Dirir (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.49\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Neil Reynolds & Lab & 3300\\
Charlotte Woods & C & 1455\\
Stef Lorenz & LD & 319\\
Daniel Towers & Grn & 236\\
Stuart Cremins & Republic & 49\\
\end{tabular*}

\subsubsection*{Claremont \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[5th October]}

\index{Claremont , Salford@Claremont, \emph{Salford}}

Death of Joe Murphy (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Mike Pevitt & Lab & 718\\
Charlotte Woods & C & 447\\
Mary Ferrer & Ind & 171\\
Stef Lorenz & LD & 162\\
Daniel Towers & Grn & 46\\
\end{tabular*}

\subsubsection*{Langworthy \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[14th December]}

\index{Langworthy , Salford@Langworthy, \emph{Salford}}

Death of Paul Longshaw (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Wilson Nkurunziza & Lab & 601\\
James Mount & C & 183\\
Jake Overend & LD & 125\\
Ian Pattinson & Grn & 72\\
Michael Felse & Ind & 55\\
\end{tabular*}

\council{Stockport}

OSAC = Old Swan Against the Cuts (standing as ``End Austerity'')

\subsubsection*{Brinnington and Central \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Brinnington and Central , Stockport@Brinnington \& Central, \emph{Stockport}}

Death of Maureen Rowles (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Becky Crawford & Lab & 3877\\
Ros Lloyd & C & 875\\
Alex Orndal & LD & 193\\
James Pelham & Grn & 170\\
John Pearson & OSAC & 56\\
\end{tabular*}

\council{Tameside}

\subsubsection*{Droylsden East \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[26th October]}

\index{Droylsden East , Tameside@Droylsden E., \emph{Tameside}}

Resignation of Jim Middleton (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
David Mills & Lab & 1064\\
Matt Stevenson & C & 577\\
Shaun Offerman & LD & 63\\
Jean Smee & Grn & 60\\
\end{tabular*}

\council{Trafford}

\subsubsection*{Broadheath \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Broadheath , Trafford@Broadheath, \emph{Trafford}}

Resignation of Louise Dagnall (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Amy Whyte & Lab & 2086\\
Dave Morgan & C & 1778\\
Simon Latham & LD & 271\\
Mike Bayley-Sanderson & UKIP & 91\\
Joe Ryan & Grn & 80\\
Stephen Farndon & Ind & 36\\
\end{tabular*}

\subsubsection*{Bucklow-St Martins \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[14th September]}

\index{Bucklow-Saint Martins , Trafford@Bucklow-St Martins, \emph{Trafford}}

Resignation of John Smith (C elected as Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Aidan Williams & Lab & 1050\\
Sarah Marland & C & 456\\
Andrew Beaumont & UKIP & 65\\
Joseph Ryan & Grn & 33\\
Simon Lepori & LD & 18\\
\end{tabular*}

\council{Wigan}

\sloppyword{WiganIC = Wigan Independent Conservatives (standing as ``Shevington Independents'')}

\subsubsection*{Astley Mosley Common \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[19th October]}

\index{Astley Mosley Common , Wigan@Astley Mosley Common, \emph{Wigan}}

Resignation of Joanne Platt MP (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Paula Wakefield & Lab & 773\\
David Stirzaker & C & 604\\
Alan Hogg & UKIP & 185\\
Stuart Thomas & LD & 73\\
Andy Prentice & Grn & 46\\
\end{tabular*}

\subsubsection*{Shevington with Lower Ground \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[14th December]}

\index{Shevington with Lower Ground , Wigan@\sloppyword{Shevington with Lower Ground, \emph{Wigan}}}

Resignation of Damian Edwardson (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.51\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Marlaine Whitham & Lab & 765\\
Janet Brown & WiganIC & 552\\
Callum Chadwick & C & 402\\
Joseph Rylance & Grn & 30\\
Joshua Hindle & LD & 15\\
\end{tabular*}

\section{Merseyside}

\council{Knowsley}

\subsubsection*{St Michaels \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[20th July]}

\index{Saint Michaels , Knowsley@St Michaels, \emph{Knowsley}}

Resignation of Victoria Lamb (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Mike Kearns & Lab & 716\\
Dean Boyle & LD & 58\\
Kirk Sandringham & Grn & 53\\
\end{tabular*}

\council{Liverpool}

\subsubsection*{Wavertree \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Wavertree , Liverpool@Wavertree, \emph{Liverpool}}

Resignation of Helen Casstles (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Clare McIntyre & Lab & 2632\\
Joe Harmer & LD & 961\\
Stuart Wood & C & 286\\
Paul Woodruff & Grn & 270\\
Angela Grant & TUSC & 83\\
\end{tabular*}

\subsubsection*{Kirkdale \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Kirkdale , Liverpool@Kirkdale, \emph{Liverpool}}

Resignation of Beatrice Fraenkel (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.6\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Lisa Gaughan & Lab & 6416\\
Chris Hall & C & 346\\
Martyn Madeley & Grn & 177\\
Robert McAllister-Bell & LD & 154\\
\end{tabular*}

\council{Sefton}

\subsubsection*{Duke's \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[2nd November; LD gain from C]}

\index{Duke's , Sefton@Duke's, \emph{Sefton}}

Resignation of Patricia Ball (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
John Pugh & LD & 1680\\
Ann Pearmain & C & 790\\
Frank Hanley & Lab & 417\\
Terry Durrance & UKIP & 69\\
Nick Senior & Grn & 45\\
\end{tabular*}

\council{Wirral}

\subsubsection*{Claughton \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Claughton , Wirral@Claughton, \emph{Wirral}}

Death of Denise Roberts (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Gillian Wood & Lab & 1761\\
David Evans & LD & 740\\
Barbara Sinclair & C & 567\\
Liz Heydon & Grn & 136\\
Beryl Jones & UKIP & 130\\
Leon Wheddon & TUSC & 27\\
\end{tabular*}

\columnbreak

\section{South Yorkshire}

\council{Barnsley}

DDIP = Demos Direct Initiative Party

\subsubsection*{Kingstone \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[28th September]}

\index{Kingstone , Barnsley@Kingstone, \emph{Barnsley}}

Resignation of Donna Green (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Joanne Murray & Lab & 740\\
John Ellis-Mourant & LD & 247\\
Lee Ogden & C & 113\\
Chris Scarfe & Grn & 87\\
Christopher Houston & BNP & 75\\
Trevor Smith & DDIP & 25\\
\end{tabular*}

\subsubsection*{Rockingham \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[14th December]}

\index{Rockingham , Barnsley@Rockingham, \emph{Barnsley}}

Resignation of Emma Dures (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Nicola Sumner & Lab & 938\\
Michael Toon & C & 272\\
Paul Nugent & LD & 199\\
\end{tabular*}

\council{Doncaster}

At the May 2017 ordinary election there was an unfilled vacancy in Norton and Askern ward due to the death of Alan Jones (Lab).
\index{Norton and Askern , Doncaster@Norton \& Askern, \emph{Doncaster}}

\council{Rotherham}

\subsubsection*{Brinsworth and Catcliffe \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[2nd February; LD gain from Lab]}

\index{Brinsworth and Catcliffe , Rotherham@\sloppyword{Brinsworth \& Catcli{ff}e, \emph{Rotherham}}}

Resignation of Andrew Roddison (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Adam Carter & LD & 2000\\
Shabana Ahmed & Lab & 519\\
Steve Webster & UKIP & 389\\
John Oliver & C & 91\\
Bex Whyman & Grn & 30\\
\end{tabular*}

\subsubsection*{Dinnington \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[2nd February; Lab gain from UKIP]}

\index{Dinnington , Rotherham@Dinnington, \emph{Rotherham}}

Resignation of Ian Finnie (UKIP).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
John Vjestica & Lab & 670\\
Lee Hunter & UKIP & 303\\
Christopher Middleton & C & 238\\
Dave Smith & Ind & 232\\
Jean Hart & Ind & 180\\
Steven Scott & Ind & 81\\
David Foulstone & Grn & 78\\
Stephen Thornley & LD & 75\\
\end{tabular*}

\council{Sheffield}

WomensEq = Women's Equality Party

Yorks = Yorkshire Party

\subsubsection*{Southey \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Southey , Sheffield@Southey, \emph{Sheffield}}

Resignation of Leigh Bramall (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.43\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Mike Chaplin & Lab & 1594\\
Philip Brown & LD & 706\\
Dave Ogle & UKIP & 593\\
Jason Southcott & C & 388\\
Andrew Hards & Grn & 97\\
Jim Warrender & Yorks & 46\\
Carol Keen & WomensEq & 40\\
\end{tabular*}

\subsubsection*{Nether Edge and Sharrow \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[22nd June]}

\index{Nether Edge and Sharrow , Sheffield@Nether Edge \& Sharrow, \emph{Sheffield}}

Resignation of Nasima Akther (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.45\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Jim Steinke & Lab & 2641\\
Rob Unwin & Grn & 2509\\
Shahid Ali & LD & 722\\
\end{tabular*}

\subsubsection*{Beighton \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[12th October]}

\index{Beighton , Sheffield@Beighton, \emph{Sheffield}}

\sloppyword{Resignation of Helen Mir{fi}n-Boukouris (Lab).}

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Sophie Wilson & Lab & 1640\\
Bob McCann & LD & 899\\
Laurence Smith & C & 552\\
Shane Harper & UKIP & 212\\
Anthony Naylor & Grn & 74\\
\end{tabular*}

\section{Tyne and Wear}

\council{Newcastle upon Tyne}

\subsubsection*{South Heaton \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[16th March]}

\index{South Heaton , Newcastle upon Tyne@South Heaton, \emph{Newcastle upon Tyne}}

Resignation of Sophie White (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Clare Penny-Evans & Lab & 768\\
Andrew Gray & Grn & 444\\
Christopher Boyle & LD & 260\\
Tony Sanderson & UKIP & 88\\
Chris Murray & C & 80\\
\end{tabular*}

\council{Sunderland}

\subsubsection*{Sandhill \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[12th January; LD gain from Lab]}

\index{Sandhill , Sunderland@Sandhill, \emph{Sunderland}}

Disqualification (non-attendance) of Jacqui Gallagher (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Stephen O'Brien & LD & 824\\
Gary Waller & Lab & 458\\
Bryan Foster & UKIP & 343\\
Gavin Wilson & C & 184\\
Helmut Izaks & Grn & 23\\
\end{tabular*}

\section{West Midlands}

\council{Birmingham}

\subsubsection*{Hall Green \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Hall Green , Birmingham@Hall Green, \emph{Birmingham}}

Resignation of Sam Burden (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Liz Clements & Lab & 3138\\
Tanveer Choudhry & LD & 1889\\
Obaid Khan & C & 1683\\
Alan Blumenthal & UKIP & 462\\
Gareth Courage & Grn & 307\\
\end{tabular*}

\subsubsection*{Perry Barr \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Perry Barr , Birmingham@Perry Barr, \emph{Birmingham}}

Death of Ray Hassall (LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Morriam Jan & LD & 2387\\
Mohammed Hanif & Lab & 2156\\
Minu Sungu & C & 558\\
Matthew Ford & Grn & 314\\
\end{tabular*}

\council{Dudley}

\subsubsection*{St Thomas's \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[16th February]}

\index{Saint Thomas's , Dudley@St Thomas's, \emph{Dudley}}

Resignation of Glenis Simms (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Shaneila Mughal & Lab & 1466\\
Phil Wimlett & UKIP & 653\\
Jonathan Elliott & C & 249\\
Francis Sheppard & Grn & 52\\
\end{tabular*}

\section{West Yorkshire}

\council{Bradford}

\subsubsection*{Queensbury \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Queensbury , Bradford@Queensbury, \emph{Bradford}}

Resignation of Lisa Carmody (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Andrew Senior & C & 2175\\
Jason Smith & Ind & 732\\
Mobeen Hussain & Lab & 697\\
Eithne Dodwell & Grn & 143\\
Mary Whitrick & LD & 135\\
\end{tabular*}

\council{Calderdale}

\subsubsection*{Hipperholme and Lightcliffe \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[6th April]}

\index{Hipperholme and Lightcliffe , Calderdale@Hipperholme \& Lightcliffe, \emph{Calderdale}}

Death of Graham Hall (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
George Robinson & C & 1483\\
Alisdair McGregor & LD & 420\\
Oliver Willows & Lab & 407\\
Elaine Hey & Grn & 150\\
\end{tabular*}

\subsubsection*{Rastrick \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Rastrick , Calderdale@Rastrick, \emph{Calderdale}}

Resignation of Lynne Mitchell (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.6\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Sophie Whittaker & C & 2929\\
Colin Hutchinson & Lab & 2466\\
Kathy Haigh-Hutchinson & LD & 298\\
Barry Crossland & Grn & 229\\
\end{tabular*}

\subsubsection*{Todmorden \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Todmorden , Calderdale@Todmorden, \emph{Calderdale}}

Resignation of Jayne Booth (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Carol Machell & Lab & 3648\\
Gilbert Gregory & C & 2076\\
Donal O'Hanlon & LD & 448\\
Angharad Turner & Grn & 446\\
\end{tabular*}

\council{Kirklees}

HWDI = Heavy Woollen District Independents

\subsubsection*{Batley East \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[26th October]}

\index{Batley East , Kirklees@Batley E., \emph{Kirklees}}

\sloppyword{Disquali{fi}cation (non-attendance) of Amanda Stubley (Ind elected as Lab).}

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Habiban Zaman & Lab & 2640\\
Paul Young & C & 443\\
Aleks Lukic & HWDI & 140\\
Jon Bloom & LD & 136\\
David Smith & Grn & 70\\
\end{tabular*}

\columnbreak

\council{Wakefield}

Yorks = Yorkshire Party

\subsubsection*{Stanley and Outwood East \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[12th October]}

\index{Stanley and Outwood East , Wakefield@Stanley \& Outwood E., \emph{Wakefield}}

Resignation of Clive Hudson (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Jack Hemingway & Lab & 1353\\
Nathan Garbutt Moore & C & 847\\
Nicola Sinclair & LD & 165\\
Lucy Brown & Yorks & 153\\
James Johnston & UKIP & 136\\
\end{tabular*}

\subsubsection*{Wakefield West \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[23rd November]}

\index{Wakefield West , Wakefield@Wakefield W., \emph{Wakefield}}

Resignation of Ryan Case (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Michael Graham & Lab & 1118\\
Dawn Hunt & C & 933\\
Paul Phelps & Yorks & 152\\
Peter Williams & LD & 46\\
\end{tabular*}

\section{Bedfordshire}

\council{Central Bedfordshire}

\subsubsection*{Biggleswade North \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Biggleswade North , Central Bedfordshire@Biggleswade N., \emph{C. Beds.}}

Resignation of Maurice Jones (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Steven Watkins & C & 704\\
Bernard Rix & Ind & 541\\
Bernard Briars & Lab & 387\\
Duncan Strachan & UKIP & 158\\
Amy Peabody & Grn & 134\\
Nigel Aldis & LD & 81\\
\end{tabular*}

\subsubsection*{Shefford \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Shefford , Central Bedfordshire@Shefford, \emph{C. Beds.}}

Death of Lewis Birt (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Mark Liddiard & C & 1325\\
Annabel Woolmer & LD & 499\\
David Short & Lab & 345\\
Roger Smith & UKIP & 267\\
\end{tabular*}

\council{Luton}

\subsubsection*{Sundon Park \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Sundon Park , Luton@Sundon Park, \emph{Luton}}

Resignation of Fiona Green (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Martin Rogers & Lab & 1919\\
Clive Mead & LD & 957\\
Morel Benard & C & 908\\
Simon Hall & Grn & 72\\
\end{tabular*}

\section{Berkshire}

\council{Slough}

\subsubsection*{Haymill and Lynch Hill \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Haymill and Lynch Hill , Slough@Haymill \& Lynch Hill, \emph{Slough}}

Death of Darren Morris (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Paul Kelly & C & 1036\\
Jemma Davis & Lab & 899\\
Gary Griffin & LD & 129\\
Paul Janik & Ind & 52\\
\end{tabular*}

\council{West Berkshire}

\subsubsection*{Thatcham South and Crookham \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Thatcham South and Crookham , West Berkshire@Thatcham S. \& Crookham, \emph{W. Berks.}}

Death of Roger Croft (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Jason Collis & C & 1823\\
Owen Jeffery & LD & 822\\
Stephen Masters & Grn & 590\\
Nicholas East & Lab & 505\\
\end{tabular*}

\council{Windsor and Maidenhead}

\subsubsection*{Clewer North \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May; Ind gain from C]}

\index{Clewer North , Windsor and Maidenhead@Clewer N., \emph{Windsor \& Maidenhead}}

Resignation of John Collins (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Wisdom da Costa & Ind & 805\\
Lars Swann & C & 784\\
Amy Tisi & LD & 405\\
Michael Boyle & Lab & 273\\
\end{tabular*}

\council{Wokingham}

\subsubsection*{Emmbrook \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[Friday 17th February; LD gain from C]}

\index{Emmbrook , Wokingham@Emmbrook, \emph{Wokingham}}

Resignation of Christopher Singleton (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.57\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Imogen Shepherd-Dubey & LD & 1575\\
Kevin Morgan & C & 879\\
Phil Ray & UKIP & 104\\
Christopher Everett & Lab & 79\\
\end{tabular*}

\section{Buckinghamshire}

\subsection*{County Council}\index{Buckinghamshire}

At the May 2017 ordinary election there was an unfilled vacancy in Amersham and Chesham Bois division due to the death of Martin Phillips (C).
\index{Amersham and Chesham Bois , Buckinghamshire@Amersham \& Chesham Bois, \emph{Bucks.}}

\subsubsection*{Beaconsfield \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[2nd November]}

\index{Beaconsfield , Buckinghamshire@Beaconsfield, \emph{Bucks}}

Death of Alan Walters (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Anita Cranmer & C & 1298\\
Mark Skoyles & LD & 299\\
\end{tabular*}

\council{Aylesbury Vale}

\subsubsection*{Elmhurst \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[6th April; LD gain from UKIP]}

\index{Elmhurst , Aylesbury Vale@Elmhurst, \emph{Aylesbury Vale}}

Resignation of Andy Hetherington (UKIP).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Susan Morgan & LD & 785\\
Gary Paxton & Lab & 151\\
Ammer Raheel & C & 147\\
Phil Gomm & UKIP & 111\\
Nigel Foster & Grn & 43\\
\end{tabular*}

\subsubsection*{Wendover and Halton \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Wendover and Halton , Aylesbury Vale@Wendover \& Halton, \emph{Aylesbury Vale}}

Resignation of Andrew Southam (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Richard Newcombe & C & 1240\\
Cath Collier & Lab & 417\\
Ashley Morgan & LD & 298\\
Des Anning & UKIP & 273\\
Tom Hodge & Grn & 227\\
\end{tabular*}

\subsubsection*{Riverside \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[17th August]}

\index{Riverside , Aylesbury Vale@Riverside, \emph{Aylesbury Vale}}

Resignation of Nick Lewis (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Ashley Waite & C & 301\\
Jason Bingley & LD & 286\\
John Cowell & Lab & 210\\
Phil Gomm & UKIP & 48\\
Mary Hodgskiss & Grn & 23\\
\end{tabular*}

\subsubsection*{Southcourt \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[17th August]}

\index{Southcourt , Aylesbury Vale@Southcourt, \emph{Aylesbury Vale}}

Resignation of Peter Agoro (LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Sally-Anne Jarvis & LD & 456\\
Akhmad Hussain & C & 386\\
Ansar Gulzar & Lab & 270\\
Julie Atkins & Grn & 58\\
Geoffrey Baile & UKIP & 54\\
\end{tabular*}

\council{Chiltern}

\subsubsection*{Great Missenden \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May; C gain from Ind]}

\index{Great Missenden , Chiltern@Great Missenden, \emph{Chiltern}}

Resignation of Seb Berry (Ind).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Vanessa Martin & C & 416\\
Ruth Yeoman & LD & 175\\
Anna Brazil & Grn & 163\\
Alan Stevens & UKIP & 26\\
\end{tabular*}

\subsubsection*{Penn and Coleshill \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[16th November]}

\index{Penn and Coleshill , Chiltern@Penn \& Coleshill, \emph{Chiltern}}

Resignation of Alan Hardie (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Jonathan Waters & C & 697\\
Richard Williams & LD & 168\\
\end{tabular*}

\council{South Bucks}

\subsubsection*{Burnham Lent Rise and Taplow \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[5th October]}

\index{Burnham Lent Rise and Taplow , South Bucks@Burnham Lent Rise \& Taplow, \emph{S. Bucks}}

Death of Alan Sansom (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Matt Bezzant & C & 699\\
Alexa Collins & Lab & 166\\
Carol Linton & LD & 136\\
Zoe Hatch & Grn & 60\\
\end{tabular*}

\subsubsection*{Beaconsfield North \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[2nd November]}

\index{Beaconsfield North , South Bucks@Beaconsfield N., \emph{S. Bucks}}

Death of Alan Walters (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Damian Saunders & C & 441\\
Paul Henry & LD & 136\\
\end{tabular*}

\section{Cambridgeshire}

\council{Cambridge}

\subsubsection*{Arbury \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Arbury , Cambridge@Arbury, \emph{Cambridge}}

Resignation of Charlotte Perry (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Patrick Sheil & Lab & 1267\\
Tim Ward & LD & 992\\
Henry Collins & C & 450\\
\end{tabular*}

\council{East Cambridgeshire}

\subsubsection*{Soham North \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[22nd June]}

\index{Soham North , East Cambridgeshire@Soham N., \emph{E. Cambs.}}

Election of James Palmer as Mayor of Cambridgeshire and Peterborough (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Mark Goldsack & C & 423\\
Alec Jones & LD & 178\\
Peter Tyson & Lab & 108\\
\end{tabular*}

\subsubsection*{Ely South \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[7th September; LD gain from C]}

\index{Ely South , East Cambridgeshire@Ely S., \emph{E. Cambs.}}

Resignation of Tom Hunt (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.63\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Christine Whelan & LD & 527\\
Sarah Bellow & C & 411\\
Rebecca Denness & Lab & 384\\
\end{tabular*}

\council{Huntingdonshire}

StNeots = St Neots Independent Group

\subsubsection*{St Neots Eaton Ford \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May; StNeots gain from C]}

\index{Saint Neots Eaton Ford , Huntingdonshire@St Neots Eaton Ford, \emph{Hunts.}}

Resignation of David Harty (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Charles Bober & StNeots & 854\\
Keith Prentice & C & 788\\
Michael Walker & LD & 353\\
\end{tabular*}

\council{Peterborough}

\subsubsection*{East \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{East , Peterborough@East, \emph{Peterborough}}

Death of Marcus Sims (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Matthew Mahabadi & Lab & 1900\\
Jay Beecher & C & 1111\\
Graham Whitehead & UKIP & 358\\
Jelana Stevic & LD & 332\\
\end{tabular*}

\subsubsection*{Park \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[17th August]}

\index{Park , Peterborough@Park, \emph{Peterborough}}

Resignation of John Shearman (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Shaz Nawaz & Lab & 1713\\
Arfan Khan & C & 1375\\
Graham Whitehead & UKIP & 176\\
Ian Hardman & LD & 109\\
Carolyn English & Grn & 83\\
\end{tabular*}

\subsubsection*{Eye, Thorney and Newborough \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[7th September]}

\index{Eye, Thorney and Newborough , Peterborough@Eye, Thorney \& Newborough, \emph{Peterborough}}

Resignation of David Sanders (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Nigel Simons & C & 1018\\
Christian DeFeo & Lab & 555\\
Mary Herdman & UKIP & 279\\
Michael Alexander & Grn & 61\\
Callum Robertson & LD & 35\\
\end{tabular*}

\council{South Cambridgeshire}

\subsubsection*{Bourn \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Bourn , South Cambridgeshire@Bourn, \emph{S. Cambs.}}

Resignation of Mervyn Loynes (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Ruth Betson & C & 1426\\
Jeni Sawford & LD & 871\\
Mark Hurn & Lab & 573\\
\end{tabular*}



\section{Cheshire}

\council{Cheshire East}

Boll1st = Bollington First

\subsubsection*{Bollington \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[16th February; Boll1st gain from C]}

\index{Bollington , Cheshire East@Bollington, \emph{Ches. E.}}

Resignation of Jon Weston (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
James Nicholas & Boll1st & 939\\
Philip Bolton & C & 319\\
Rob Vernon & Lab & 239\\
Sam al-Hamdani & LD & 198\\
Richard Purslow & Grn & 162\\
\end{tabular*}

\subsubsection*{Crewe East \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[5th October]}

\index{Crewe East , Cheshire East@Crewe E., \emph{Ches. E.}}

Resignation of David Newton (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Joy Bratherton & Lab & 1174\\
Mary Addison & C & 542\\
Eddie Ankers & UKIP & 158\\
Melanie English & Grn & 59\\
\end{tabular*}

\council{Cheshire West and Chester}

\subsubsection*{Blacon \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[20th April]}

\index{Blacon , Cheshire West and Chester@Blacon, \emph{Ches. W. \& Chester}}

Resignation of Reg Jones (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Ben Powell & Lab & 1556\\
Jack Jackson & C & 574\\
Steve Ingram & Ind & 434\\
Lizzie Jewkes & LD & 70\\
\end{tabular*}

\council{Warrington}

\subsubsection*{Chapelford and Old Hall \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[12th October]}

\index{Chapelford and Old Hall , Warrington@Chapelford \& Old Hall, \emph{Warrington}}

Resignation of Faisal Rashid MP (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Paul Warburton & Lab & 957\\
Phil Hayward & C & 333\\
Allan Bird & LD & 312\\
Ian Wilson & UKIP & 86\\
Stephanie Davies & Grn & 43\\
\end{tabular*}

\section{Cornwall}

\council{Cornwall}

\subsubsection*{Bodmin St Petroc \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Bodmin Saint Petroc , Cornwall@Bodmin St Petroc, \emph{Cornwall}}

Ordinary election postponed from 4th May: death of outgoing councillor Steve Rogerson (LD) who had been nominated for re-election.

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Leigh Frost & LD & 1047\\
Sylvia Berry & C & 914\\
Marc Thorne & Lab & 463\\
John Gibbs & MK & 231\\
\end{tabular*}

\section{Cumbria}

\council{Carlisle}

\subsubsection*{Belle Vue \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Belle Vue , Carlisle@Belle Vue, \emph{Carlisle}}

Resignation of Jacqui Franklin (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Pamela Birks & Lab & 775\\
Rob Currie & C & 771\\
Susan Riley & UKIP & 116\\
\end{tabular*}

\subsubsection*{Yewdale \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May; C gain from Lab]}

\index{Yewdale , Carlisle@Yewdale, \emph{Carlisle}}

Resignation of Tom Dodd (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Christina Finlayson & C & 931\\
David Graham & Lab & 792\\
Jeff Coates & LD & 152\\
Malcolm Craik & UKIP & 92\\
\end{tabular*}

\council{Copeland}

\subsubsection*{Newtown \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Newtown , Copeland@Newtown, \emph{Copeland}}

Death of Ray Cole (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Ged McGrath & C & 954\\
Angela Dixon & Lab & 601\\
\end{tabular*}

\subsubsection*{Egremont South \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[2nd November]}

\index{Egremont South , Copeland@Egremont S., \emph{Copeland}}

Resignation of Lena Hogg (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Tom Higgins & Lab & 354\\
Jeff Hailes & C & 321\\
\end{tabular*}

\council{Eden}

\subsubsection*{Alston Moor \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[20th July; Lab gain from LD]}

\index{Alston Moor , Eden@Alston Moor, \emph{Eden}}

Resignation of Thomas Sheriff (LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Lissie Sharp & Lab & 407\\
Jim Clapp & C & 253\\
Holly Ho & Ind & 57\\
Richard O'Brien & Grn & 13\\
\end{tabular*}

\subsubsection*{Penrith North \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[16th November]}

\index{Penrith North , Eden@Penrith N., \emph{Eden}}

Resignation of Robin Howse (LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Mark Rudhall & LD & 422\\
John Forrester & C & 291\\
Karen Lockney & Lab & 155\\
Douglas Lawson & Grn & 65\\
\end{tabular*}

\council{South Lakeland}

\subsubsection*{Ambleside and Grasmere \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Ambleside and Grasmere , South Lakeland@Ambleside \& Grasmere, \emph{S. Lakeland}}

Resignation of Heidi Halliday (LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Vicky Hughes & LD & 695\\
Tim Brown & C & 502\\
Alison Gilchrist & Lab & 87\\
Chris Rowley & Grn & 82\\
\end{tabular*}

\section{Derbyshire}

\council{Chesterfield}

CANDI = Chesterfield and North Derbyshire Independents

\subsubsection*{Holmebrook \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[21st September; LD gain from Lab]}

\index{Holmebrook , Chesterfield@Holmebrook, \emph{Chesterfield}}

Resignation of Stephen Hitchin (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Keith Falconer & LD & 510\\
Ron Mihaly & Lab & 435\\
Oliver Scheidt & C & 62\\
Paul Stone & CANDI & 14\\
\end{tabular*}

\council{Derby}

\subsubsection*{Derwent \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[9th March; C gain from UKIP]}

\index{Derwent , Derby@Derwent, \emph{Derby}}

Death of Bill Wright (UKIP).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Steve Willoughby & C & 789\\
Nadine Peatfield & Lab & 611\\
Tony Crawley & UKIP & 537\\
Simon Ferrigno & LD & 192\\
\end{tabular*}

\council{Derbyshire Dales}

\subsubsection*{Ashbourne South \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[26th October]}

\index{Ashbourne South , Derbyshire Dales@Ashbourne S., \emph{Derbys. Dales}}

Resignation of Phil Chell (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Dermot Murphy & C & 495\\
Rebecca Goodall & LD & 334\\
Andy White & Lab & 242\\
\end{tabular*}

\council{High Peak}

\subsubsection*{Limestone Peak \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[9th November]}

\index{Limestone Peak , High Peak@Limestone Peak, \emph{High Peak}}

Resignation of Daren Robins (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Peter Roberts & C & 261\\
Jim Lambert & Lab & 133\\
Alistair Forbes & LD & 58\\
Peter Crook & Grn & 34\\
\end{tabular*}

\council{South Derbyshire}

\subsubsection*{Woodville \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[11th May]}

\index{Woodville , South Derbyshire@Woodville, \emph{S. Derbys.}}

Death of Gill Farrington (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Raymond Tipping & C & 613\\
Malc Gree & Lab & 510\\
Mike Dawson & UKIP & 118\\
Rebecca Wilkinson & LD & 82\\
\end{tabular*}

\section{Devon}

\council{East Devon}

\subsubsection*{Broadclyst \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May; LD gain from C]}

\index{Broadclyst , East Devon@Broadclyst, \emph{E. Devon}}

Resignation of Maria Hale (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Eleanor Rylance & LD & 1045\\
Ray Bloxham & C & 953\\
\end{tabular*}

\subsubsection*{Whimple \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Whimple , East Devon@Whimple, \emph{E. Devon}}

Resignation of Peter Bowden (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Mark Evans-Martin & C & 455\\
Martin Gammell & LD & 419\\
\end{tabular*}

\council{Exeter}

\subsubsection*{St Thomas \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Saint Thomas , Exeter@St Thomas, \emph{Exeter}}

Death of Paul Bull (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Heather Morris & Lab & 1370\\
Adrian Fullam & LD & 863\\
Alex Traves & C & 628\\
Joe Levy & Grn & 197\\
Alison Sheridan & UKIP & 173\\
\end{tabular*}

\subsubsection*{Newtown and St Leonard's \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[Wednesday 13th December]}

\index{Newtown and Saint Leonard's , Exeter@Newtown \& St Leonard's, \emph{Exeter}}

Resignation of Roger Spackman (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Matthew Vizard & Lab & 1044\\
Lucille Baker & C & 512\\
Alexandra Newcombe & LD & 179\\
Tom Milburn & Grn & 137\\
Alison Sheridan & UKIP & 40\\
\end{tabular*}

\council{Mid Devon}

\subsubsection*{Westexe \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[14th September; C gain from UKIP]}

\index{Westexe , Mid Devon@Westexe, \emph{Mid Devon}}

Resignation of Jonathan Smith (Ind elected as UKIP).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Anthony Bush & C & 279\\
Gerald Luxton & Ind & 179\\
Alison Mitchell & Lab & 164\\
David Whiteway & LD & 144\\
\end{tabular*}

\council{North Devon}

\subsubsection*{Braunton East \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[2nd November; LD gain from C]}

\index{Braunton East , North Devon@Braunton E., \emph{N. Devon}}

Resignation of Roy Lucas (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Derrick Spear & LD & 459\\
Brad Bunyard & Grn & 387\\
Felix Milton & C & 225\\
Mark Cann & Lab & 165\\
\end{tabular*}

\subsubsection*{Newport \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[7th December; LD gain from C]}

\index{Newport , North Devon@Newport, \emph{N. Devon}}

Death of Michael Harrison (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Caroline Leaver & LD & 390\\
Martin Kennaugh & C & 373\\
Ricky Knight & Grn & 159\\
Siobhan Strode & Lab & 83\\
\end{tabular*}

\council{South Hams}

\subsubsection*{Charterlands \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[23rd February; LD gain from C]}

\index{Charterlands , South Hams@Charterlands, \emph{S. Hams}}

Resignation of Lindsay Ward (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Elizabeth Huntley & LD & 473\\
Jonathan Bell & C & 404\\
David Trigger & Lab & 110\\
Janet Chapman & Grn & 40\\
\end{tabular*}

\council{Teignbridge}

\subsubsection*{Bushell \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May; LD gain from C]}

\index{Bushell , Teignbridge@Bushell, \emph{Teignbridge}}

Resignation of Judy Grainger (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Robert Hayes & LD & 751\\
Liz Roberts & C & 627\\
Steven Harvey & UKIP & 158\\
\end{tabular*}

\subsubsection*{Kingsteignton East \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Kingsteignton East , Teignbridge@Kingsteignton E., \emph{Teignbridge}}

Resignation of Mike Walters (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Ron Peart & C & 778\\
Sue Rollason & LD & 437\\
Tony Dempster & Ind & 270\\
\end{tabular*}

\council{Torbay}

\subsubsection*{Watcombe \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[14th December]}

\index{Watcombe , Torbay@Watcombe, \emph{Torbay}}

Death of Roger Stringer (LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Swithin Long & LD & 655\\
Daniel Maddock & C & 355\\
Julia Neal & Lab & 121\\
Eleanor Taylor & Grn & 17\\
\end{tabular*}

\council{Torridge}

\subsubsection*{Hartland and Bradworthy \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June; LD gain from UKIP]}

\index{Hartland and Bradworthy , Torridge@Hartland \& Bradworthy, \emph{Torridge}}

Death of Alan Whittle (UKIP).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Jane Leaper & LD & 973\\
John Sanders & Grn & 720\\
\end{tabular*}

\subsubsection*{Torrington \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[30th November; LD gain from UKIP]}

\index{Torrington , Torridge@Torrington, \emph{Torridge}}

Resignation of Roger Darch (Ind elected as UKIP).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Cheryl Cottle-Hunkin & LD & 701\\
Di Davey & Ind & 180\\
Harold Martin & C & 159\\
Sue Clarke & Grn & 76\\
John Pitts & UKIP & 49\\
\end{tabular*}

\subsubsection*{Westward Ho! \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[14th December; Ind gain from C]}

\index{Westward Ho"! , Torridge@Westward Ho"!, \emph{Torridge}}

Resignation of Peter le Maitre (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Nick Laws & Ind & 321\\
Roger Levick & C & 128\\
Derek Sargent & UKIP & 90\\
Stephen Potts & LD & 63\\
Barry Mason & Ind & 47\\
Stan Coats & Lab & 35\\
\end{tabular*}

\council{West Devon}

\subsubsection*{Bere Ferrers \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Bere Ferrers , West Devon@Bere Ferrers, \emph{W. Devon}}

Death of Mike Benson (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Brian Lamb & C & 639\\
Katherine Medhurst & Lab & 421\\
David Pengelly & UKIP & 164\\
\end{tabular*}

\section{Dorset}

\council{Christchurch}

\subsubsection*{Mudeford and Friars Cliff \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[2nd March]}

\index{Mudeford and Friars Cliff , Christchurch@Mudeford \& Friars Cliff, \emph{Christchurch}}

Resignation of Andy Barfield (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Paul Hilliard & C & 629\\
Sheila Gray & Ind & 466\\
Julian Spurr & Lab & 91\\
Lawrence Wilson & UKIP & 85\\
Fiona Cownie & Grn & 72\\
\end{tabular*}

\council{North Dorset}

\subsubsection*{Gillingham Town \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May; C gain from LD]}

\index{Gillingham Town , North Dorset@Gillingham Town, \emph{N. Dorset}}

Resignation of David Milsted (LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Alexander Chase & C & 678\\
Barry von Clemens & LD & 660\\
Peter Caulfield & UKIP & 129\\
\end{tabular*}

\subsubsection*{Blandford Central \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[27th July; C gain from Ind]}

\index{Blandford Central , North Dorset@Blandford C., \emph{N. Dorset}}

Death of Esme Butler (Ind).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Noc Lacey-Clarke & C & 310\\
Haydn White & Lab & 307\\
Hugo Mieville & LD & 229\\
\end{tabular*}

\council{West Dorset}

\subsubsection*{Piddle Valley \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[13th April]}

\index{Piddle Valley , West Dorset@Piddle Valley, \emph{W. Dorset}}

Resignation of Peter Hiscock (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Brian Haynes & C & 303\\
Carol Byrom & Grn & 195\\
\end{tabular*}

\subsubsection*{Lyme Regis and Charmouth \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[14th September; Ind gain from C]}

\index{Lyme Regis and Charmouth , West Dorset@Lyme Regis \& Charmouth, \emph{W. Dorset}}

Resignation of George Symonds (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Cheryl Reynolds & Ind & 622\\
Paul Oatway & C & 396\\
Belinda Bawden & Lab & 171\\
\end{tabular*}

\council{Weymouth and Portland}

\subsubsection*{Westham East \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May; C gain from LD]}

\index{Westham East , Weymouth and Portland@Westham E., \emph{Weymouth \& Portland}}

Resignation of Sally Maslin (LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Mike Byatt & C & 294\\
Andreas Scheffler & LD & 270\\
Caz Dennett & Grn & 260\\
David Greenhalf & Lab & 236\\
\end{tabular*}

\columnbreak

\section{Durham}

\council{Darlington}

\subsubsection*{Mowden \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[16th November]}

\index{Mowden , Darlington@Mowden, \emph{Darlington}}

Resignation of Bill Stenson (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Alan Marshall & C & 652\\
Eddie Heslop & Lab & 285\\
Sarah Jordan & LD & 111\\
Kathy Barley & Grn & 26\\
\end{tabular*}

\subsubsection*{Red Hall and Lingfield \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[16th November]}

\index{Red Hall and Lingfield , Darlington@Red Hall \& Lingfield, \emph{Darlington}}

Resignation of Lynne Haszeldine (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Sharifah Rahman & Lab & 249\\
Jonathan Dulston & C & 230\\
Kevin Brack & Ind & 46\\
Mike McTimoney & Grn & 20\\
Harry Longmoor & LD & 11\\
\end{tabular*}

\council{Durham}

At the May 2017 ordinary election there were unfilled vacancies in Chester-le-Street East and Coxhoe divisions due to the resignations of Lawson Armstrong and Mac Williams (both Lab) respectively.
\index{Chester-le-Street East , Durham@Chester-le-Street E., \emph{Durham}}
\index{Coxhoe , Durham@Coxhoe, \emph{Durham}}

Seaham = Seaham Community Party

\subsubsection*{Dawdon \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[29th June]}

\index{Dawdon , Durham@Dawdon, \emph{Durham}}

Resignation of Leanne Kennedy (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Leanne Kennedy & Lab & 693\\
Bob Arthur & Seaham & 633\\
\end{tabular*}

\subsubsection*{Trimdon and Thornley \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[28th September]}

\index{Trimdon \& Thornley , Durham@Trimdon \& Thornley, \emph{Durham}}

Death of Morris Nicholls (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Jude Grant & Lab & 1150\\
Maurice Brown & Ind & 351\\
Alan Bell & LD & 117\\
Michael Smith & C & 112\\
Jonathan Elmer & Grn & 29\\
\end{tabular*}

\council{Hartlepool}

PHF = Putting Hartlepool First

\subsubsection*{Headland and Harbour \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Headland and Harbour , Hartlepool@Headland \& Harbour, \emph{Hartlepool}}

Resignation of Sylvia Tempest (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Mike McLaughlin & Lab & 555\\
Tom Cassidy & UKIP & 532\\
Chris Broadbent & C & 210\\
Lucy Patterson & Ind & 69\\
\end{tabular*}

\subsubsection*{Seaton \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[19th October; PHF gain from Ind]}

\index{Seaton , Hartlepool@Seaton, \emph{Hartlepool}}

Resignation of Paul Thompson (Ind).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Leisa Smith & PHF & 474\\
Sue Little & Ind & 425\\
Ann Marshall & Lab & 275\\
Mike Young & C & 180\\
Karen King & UKIP & 148\\
\end{tabular*}

\subsubsection*{Victoria \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[16th November]}

\index{Victoria , Hartlepool@Victoria, \emph{Hartlepool}}

Resignation of Trisha Lawton (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Katie Trueman & Lab & 479\\
Jacqui Cummings & UKIP & 325\\
Andrew Martin-Wells & C & 98\\
\end{tabular*}

\council{Stockton-on-Tees}

NEast = North East Party

\subsubsection*{Newtown \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[18th May]}

\index{Newtown , Stockton-on-Tees@Newtown, \emph{Stockton-on-Tees}}

Resignation of Rachael Proud (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Marilyn Surtees & Lab & 483\\
Miguel Rodrigues & C & 201\\
David Kirk & Ind & 193\\
Jason Rossiter & LD & 43\\
\end{tabular*}

\subsubsection*{Yarm \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[22nd June]}

\index{Yarm , Stockton-on-Tees@Yarm, \emph{Stockton-on-Tees}}

Election of Ben Houchen (C) as Mayor of the Tees Valley Combined Authority.

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.545\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Tony Hampton & C & 1179\\
Andrew Sherris & Ind & 677\\
Kevin Nicholas & Lab & 394\\
Graham Robinson & LD & 73\\
\end{tabular*}

\subsubsection*{Billingham North \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[20th July]}

\index{Billingham North , Stockton-on-Tees@Billingham N., \emph{Stockton-on-Tees}}

Resignation of Stephen Parry (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Paul Weston & Lab & 719\\
Sam Linley & C & 687\\
Jennifer Apedaile & Ind & 196\\
David Minchella & LD & 95\\
Mark Burdon & NEast & 80\\
\end{tabular*}

\subsubsection*{Parkfield and Oxbridge \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[23rd November]}

\index{Parkfield and Oxbridge , Stockton-on-Tees@Parkfield \& Oxbridge, \emph{Stockton-on-Tees}}

Resignation of Allan Mitchell (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Louise Baldock & Lab & 727\\
Aidan Cockerill & C & 409\\
Shakeel Noor & Ind & 156\\
Drew Durning & LD & 37\\
\end{tabular*}

\section{East Sussex}

\council{Lewes}

\subsubsection*{Seaford West \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Seaford West , Lewes@Seaford W., \emph{Lewes}}

Resignation of Rob Blackman (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Liz Boorman & C & 1573\\
Cheryl White & LD & 806\\
Barbara Hayes & Lab & 363\\
Eric Woodward & UKIP & 152\\
Zoe Ford & Grn & 141\\
\end{tabular*}

\subsubsection*{Ouse Valley and Ringmer \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[7th September; Grn gain from C]}

\index{Ouse Valley and Ringmer , Lewes@Ouse Valley \& Ringmer, \emph{Lewes}}

Death of Paul Gander (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Johnny Denis & Grn & 835\\
Clare Herbert & C & 660\\
James Gardiner & LD & 457\\
Tim Telford & Lab & 167\\
Phil Howson & UKIP & 38\\
\end{tabular*}

\council{Wealden}

\subsubsection*{Chiddingly and East Hoathly \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[20th July]}

\index{Chiddingly and East Hoathly , Wealden@Chiddingly \& East Hoathly, \emph{Wealden}}

Disqualification (non-attendance) of Barby Dashwood-Morris (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
David Watts & C & 349\\
Tony Fielding & Lab & 185\\
Paul Holbrook & LD & 120\\
\end{tabular*}

\section{Essex}

\subsection*{County Council}\index{Essex}

At the May 2017 ordinary election there was an unfilled vacancy in Halstead division due to the death of Joe Pike (C).
\index{Halstead , Essex@Halstead, \emph{Essex}}

\council{Chelmsford}

\subsubsection*{Moulsham and Central \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Moulsham and Central , Chelmsford@Moulsham \& Central, \emph{Chelmsford}}

Resignation of Victoria Camp (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Gilbert Smith & C & 1108\\
Graham Pooley & LD & 992\\
Edward Massey & Lab & 411\\
Tony Lane & Grn & 210\\
\end{tabular*}

\council{Colchester}

\subsubsection*{Shrub End \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[7th September; C gain from LD]}

\index{Shrub End , Colchester@Shrub End, \emph{Colchester}}

Resignation of Karen Chaplin (LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Vic Flores & C & 681\\
Mike Dale & Lab & 572\\
Sam McCarthy & LD & 373\\
Mike Clark & Ind & 54\\
Bruno Hickman & UKIP & 52\\
Victoria Weaver & Grn & 34\\
\end{tabular*}

\council{Epping Forest}

\subsubsection*{Chigwell Village \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[23rd February]}

\index{Chigwell Village , Epping Forest@Chigwell Village, \emph{Epping Forest}}

Resignation of Lesley Wagland (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.62\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Darshan Singh Sunger & C & 453\\
Joanne Alexander-Sefre & LD & 143\\
\end{tabular*}

\subsubsection*{Lower Sheering \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[19th October]}

\index{Lower Sheering , Epping Forest@Lower Sheering, \emph{Epping Forest}}

Death of Gary Waller (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Paul Stalker & C & 220\\
Ingrid Black & LD & 52\\
\end{tabular*}

\council{Harlow}

\subsubsection*{Toddbrook \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[28th September]}

\index{Toddbrook , Harlow@Toddbrook, \emph{Harlow}}

Resignation of Rod Truan (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Tony Edwards & Lab & 702\\
Peter Lamb & C & 486\\
Patsy Long & UKIP & 98\\
James Aicken & Grn & 41\\
Lesley Rideout & LD & 19\\
\end{tabular*}

\council{Southend-on-Sea}

\subsubsection*{Shoeburyness \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[25th May]}

\index{Shoeburyness , Southend-on-Sea@Shoeburyness, \emph{Southend-on-Sea}}

Death of Mike Assenheim (Ind).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Anne Chalk & Ind & 886\\
Val Jarvis & C & 830\\
Maggie Kelly & Lab & 381\\
Edward McNally & UKIP & 121\\
Gavin Spencer & LD & 119\\
Paul Hill & Grn & 48\\
\end{tabular*}

\council{Tendring}

\subsubsection*{Great and Little Oakley \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[9th February; UKIP gain from Ind]}

\index{Great and Little Oakley , Tendring@Great \& Little Oakley, \emph{Tendring}}

Resignation of Tom Howard (Ind).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Mike Bush & UKIP & 216\\
Andy Erskine & C & 171\\
Robert Shephard & Lab & 117\\
Matthew Bensilum & LD & 83\\
\end{tabular*}

\subsubsection*{St James \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[6th April; C gain from UKIP]}

\index{Saint James , Tendring@St James, \emph{Tendring}}

Death of John Hughes (Ind elected as UKIP).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Maurice Alexander & C & 371\\
Teresa O'Hara & UKIP & 174\\
Wendy Brown & Lab & 116\\
Sean Duffy & LD & 99\\
Rosemary Dodds & Grn & 15\\
\end{tabular*}

\council{Uttlesford}

R4U = Residents for Uttlesford

\subsubsection*{Elsenham and Henham (2) \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[16th February; 2 R4U gains from LD]}

\index{Elsenham and Henham , Uttlesford@Elsenham \& Henham, \emph{Uttlesford}}

Resignations of Rory Gleeson and Lizzie Parr (both LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Petrina Lees & R4U & 834\\
Garry LeCount & R4U & 716\\
Sinead Holland & LD & 316\\
Lorraine Flawn & LD & 259\\
Joe Rich & C & 141\\
Alexis Beeching & C & 120\\
Sharron Coker & UKIP & 68\\
David Allum & UKIP & 64\\
Hilary Todd & Lab & 39\\
Carl Steward & Lab & 28\\
Paul Allington & Grn & 8\\
Karmel Stannard & Grn & 6\\
\end{tabular*}

\subsubsection*{Newport \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Newport , Uttlesford@Newport, \emph{Uttlesford}}

Resignation of Joanna Parry (R4U).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Anthony Gerrard & R4U & 623\\
Paul Simper & C & 381\\
Jayne Pacey & LD & 325\\
\end{tabular*}

\section{Gloucestershire}

\subsection*{County Council}\index{Gloucestershire}

At the May 2017 ordinary election there was an unfilled vacancy in Drybrook and Lydbrook division due to the resignation of Colin Guyton (Ind elected as UKIP).
\index{Drybrook and Lydbrook , Gloucestershire@Drybrook \& Lydbrook, \emph{Glos.}}

\council{Cheltenham}

\subsubsection*{All Saints \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{All Saints , Cheltenham@All SS., \emph{Cheltenham}}

Resignation of Dan Murch (LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Alex Hegenbarth & LD & 806\\
Ben Carlton & C & 512\\
Adrian Becker & Grn & 130\\
Joanna Hughes & Lab & 105\\
\end{tabular*}

\council{Cotswold}

\subsubsection*{Fairford North \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[9th February; LD gain from C]}

\index{Fairford North , Cotswold@Fairford N., \emph{Cotswold}}

Resignation of Abagail Beccle (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Andrew Doherty & LD & 610\\
Dom Morris & C & 270\\
Xanthe Messenger & Grn & 16\\
\end{tabular*}

\subsubsection*{Grumbolds Ash with Avening \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[23rd November]}

\index{Grumbolds Ash with Avening , Cotswold@Grumbolds Ash with Avening, \emph{Cotswold}}

Death of Jim Parsons (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Richard Morgan & C & 420\\
Nicky Baber & LD & 136\\
Edward Shelton & Lab & 93\\
\end{tabular*}

\council{Forest of Dean}

\subsubsection*{Lydbrook and Ruardean \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[16th February; Grn gain from UKIP]}

\index{Lydbrook and Ruardean , Forest of Dean@Lydbrook \& Ruardean, \emph{Forest of Dean}}

Resignation of Colin Guyton (Ind elected as UKIP).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Sid Phelps & Grn & 360\\
Kevin White & C & 248\\
Karen Brown & Lab & 231\\
Roy Bardo & UKIP & 113\\
Heather Lusty & LD & 67\\
\end{tabular*}

\council{South Gloucestershire}

\subsubsection*{Winterbourne \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[22nd June]}

\index{Winterbourne , South Gloucestershire@Winterbourne, \emph{S. Glos.}}

Election of Tim Bowles (C) as Mayor of the West of England.

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Nic Labuschagne & C & 873\\
George Angus & Lab & 615\\
Peter Bruce & LD & 333\\
\end{tabular*}

\council{Stroud}

\subsubsection*{Chalford \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[23rd November]}

\index{Chalford , Stroud@Chalford, \emph{Stroud}}

Death of Elizabeth Peters (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Darren Loftus & C & 751\\
Karen Pitney & Lab & 427\\
Robin Lewis & Grn & 341\\
Kris Beacham & LD & 146\\
\end{tabular*}

\section{Hampshire}

\subsection*{County Council}\index{Hampshire}

At the May 2017 ordinary election there was an unfilled vacancy in Bedhampton and Leigh Park division due to the resignation of Ray Finch (UKIP).
\index{Bedhampton and Leigh Park , Hampshire@Bedhampton \& Leigh Park, \emph{Hants.}}

\council{Basingstoke and Deane}

\subsubsection*{Winklebury \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[Tuesday 21st February; Lab gain from C]}

\index{Winklebury , Basingstoke and Deane@Winklebury, \emph{Basingstoke \& Deane}}

Resignation of Joseph Smith (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Angie Freeman & Lab & 824\\
Chris Hendon & C & 472\\
Zoe-Marie Rogers & LD & 42\\
\end{tabular*}

\council{Eastleigh}

\subsubsection*{Eastleigh Central \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Eastleigh Central , Eastleigh@Eastleigh C., \emph{Eastleigh}}

Resignation of Keith Trenchard (LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Tina Campbell & LD & 1431\\
Simon Payne & C & 501\\
Andy Moore & UKIP & 458\\
Steve Phillips & Lab & 452\\
\end{tabular*}

\subsubsection*{Hedge End Grange Park \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[29th June]}

\index{Hedge End Grange Park , Eastleigh@Hedge End Grange Park, \emph{Eastleigh}}

Resignation of Louise Bloom (LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Clifford Morris & LD & 668\\
Jay Haythorne & C & 316\\
Keith Day & Lab & 144\\
Rosanna Campbell & Grn & 41\\
Peter House & UKIP & 14\\
\end{tabular*}

\council{Fareham}

\subsubsection*{Stubbington \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[9th November; LD gain from UKIP]}

\index{Stubbington , Fareham@Stubbington, \emph{Fareham}}

Resignation of Chris Wood (C elected as UKIP).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Jim Forrest & LD & 1185\\
Pal Hayre & C & 769\\
Andy Annear & UKIP & 117\\
Matthew Randall & Lab & 76\\
\end{tabular*}

\council{Gosport}

\subsubsection*{Bridgemary North \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[30th November; LD gain from Lab]}

\index{Bridgemary North , Gosport@Bridgemary N., \emph{Gosport}}

Death of Jill Wright (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Stephen Hammond & LD & 644\\
James Fox & Lab & 255\\
Richard Dickson & C & 212\\
\end{tabular*}

\council{Havant}

\subsubsection*{Emsworth \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Emsworth , Havant@Emsworth, \emph{Havant}}

Death of Colin Mackey (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Richard Kennett & C & 2020\\
Crispin Ward & LD & 663\\
Steve Bilbe & Lab & 509\\
Alex Spurge & UKIP & 202\\
\end{tabular*}

\council{Rushmoor}

\subsubsection*{St John's \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Saint John's , Rushmoor@St John's, \emph{Rushmoor}}

Resignation of Peter Moyle (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Jonathan Canty & C & 870\\
Chris Harding & UKIP & 305\\
Sue Gadsby & Lab & 262\\
Charlie Fraser-Fleming & LD & 190\\
\end{tabular*}

\council{Test Valley}

\subsubsection*{Valley Park \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Valley Park , Test Valley@Valley Park, \emph{Test Valley}}

Resignation of Kath Tilling (LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Christopher Thom & LD & 1447\\
Roger Curtis & C & 963\\
\end{tabular*}



\section{Herefordshire}
\index{Herefordshire}

IOCH = It's Our County (Herefordshire)

\subsubsection*{Leominster South \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[23rd March; Grn gain from Ind]}

\index{Leominster South , Herefordshire@Leominster S., \emph{Herefs.}}

Death of Peter McCaull (Ind).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Trish Marsh & Grn & 318\\
Jon Stannard & IOCH & 143\\
Connor Egan & C & 139\\
Mark Latimer & Ind & 116\\
Clive Thomas & LD & 64\\
\end{tabular*}

\subsubsection*{Golden Valley South \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[7th September; Ind gain from C]}

\index{Golden Valley South , Herefordshire@Golden Valley S., \emph{Herefs.}}

Resignation of Graham Powell (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.63\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Peter Jinman & Ind & 462\\
Simeon Cole & C & 254\\
Richard Baker & Ind & 152\\
Jeremy Milln & Grn & 109\\
Anna Coda & Lab & 104\\
\end{tabular*}

\subsubsection*{Kings Acre \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[26th October; C gain from IOCH]}

\index{Kings Acre , Herefordshire@Kings Acre, \emph{Herefs.}}

Resignation of Mark Mansell (IOCH).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Stuart Anderson & C & 302\\
Clare Fenton & Ind & 162\\
Matt Bushkes & IOCH & 156\\
Lucy Hurds & LD & 90\\
David Lewer & Lab & 75\\
\end{tabular*}

\subsubsection*{Bishops Frome and Cradley \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[23rd November; Grn gain from C]}

\index{Bishops Frome and Cradley , Herefordshire@Bishops Frome \& Cradley, \emph{Herefs.}}

Resignation of Patricia Morgan (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Ellie Chowns & Grn & 471\\
Robert Carter & C & 299\\
Jeanie Falconer & LD & 251\\
Roger Page & Lab & 19\\
\end{tabular*}

\section{Hertfordshire}

\subsection*{County Council}\index{Hertfordshire}

At the May 2017 ordinary election there was an unfilled vacancy in Broadwater division due to the death of Sherma Batson (Lab).
\index{Broadwater , Hertfordshire@Broadwater, \emph{Herts.}}

\council{Broxbourne}

\subsubsection*{Waltham Cross \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[9th March; C gain from Lab]}

\index{Waltham Cross , Broxbourne@Waltham Cross, \emph{Broxbourne}}

Death of Malcolm Aitken (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Patsy Spears & C & 650\\
Christian Durugo & Lab & 646\\
Steve Coster & UKIP & 200\\
Brendan Wyer & LD & 89\\
\end{tabular*}

\council{Dacorum}

\subsubsection*{Berkhamsted West \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Berkhamsted West , Dacorum@Berkhamsted W., \emph{Dacorum}}

Resignation of Julian Ashbourn (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Anthony Armytage & C & 753\\
Sally Symington & LD & 706\\
Philip Parry & Lab & 178\\
Mary Hardinge & Grn & 161\\
\end{tabular*}

\subsubsection*{Bovingdon, Flaunden and Chipperfield \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Bovingdon, Flaunden and Chipperfield , Dacorum@Bovingdon, Flaunden \& Chipperfield, \emph{Dacorum}}

Resignation of Adam Barnes (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Graham Barrett & C & 3313\\
Michael Chapman & LD & 1646\\
\end{tabular*}

\council{East Hertfordshire}

\subsubsection*{Hertford Castle \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[9th March]}

\index{Hertford Castle , East Hertfordshire@Hertford Castle, \emph{E. Herts.}}

Resignation of Kevin Brush (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Linda Radford & C & 593\\
Veronica Fraser & Lab & 207\\
Freya Waterhouse & LD & 188\\
Tony Tarrega & Grn & 157\\
Mike Shaw & UKIP & 65\\
\end{tabular*}

\subsubsection*{Datchworth and Aston \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Datchworth and Aston , East Hertfordshire@Datchworth \& Aston, \emph{E. Herts.}}

Resignation of Anthony Jackson (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Tony Stowe & C & 668\\
Victoria Jordan & LD & 197\\
\end{tabular*}

\council{Hertsmere}

\subsubsection*{Potters Bar Oakmere \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Potters Bar Oakmere , Hertsmere@Potters Bar Oakmere, \emph{Hertsmere}}

Resignation of Penny Swallow (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Sarah Hodgson-Jones & C & 882\\
John Doolan & Lab & 445\\
Vikki Johnson & UKIP & 163\\
\end{tabular*}

\subsubsection*{Borehamwood Kenilworth \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[5th October; Lab gain from C]}

\index{Borehamwood Kenilworth , Hertsmere@Borehamwood Kenilworth, \emph{Hertsmere}}

Resignation of Thomas Ash (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Jeremy Newmark & Lab & 383\\
Pat Strack & C & 341\\
Elaine Elliman & LD & 144\\
Lawrence Stack & Ind & 91\\
Vikki Johnson & UKIP & 54\\
\end{tabular*}

\council{North Hertfordshire}

\subsubsection*{Hitchin Priory \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Hitchin Priory , North Hertfordshire@Hitchin Priory, \emph{N. Herts.}}

Resignation of Allison Ashley (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Raymond Shakespeare-Smith & C & 717\\
John Hefford & LD & 394\\
Jacqueline McDonald & Ind & 277\\
Dave Winstanley & Lab & 219\\
Des Stephens & Grn & 114\\
\end{tabular*}

\subsubsection*{Royston Heath \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Royston Heath , North Hertfordshire@Royston Heath, \emph{N. Herts.}}

Death of Peter Burt (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Sarah Dingley & C & 844\\
Carol Stanier & LD & 470\\
Vaughan West & Lab & 196\\
Katherine Shann & Grn & 97\\
Christopher Arquati & UKIP & 68\\
\end{tabular*}

\council{Stevenage}

\subsubsection*{Roebuck \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May; C gain from Lab]}

\index{Roebuck , Stevenage@Roebuck, \emph{Stevenage}}

Death of Sherma Batson (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Alexander Farquharson & C & 725\\
Monika Cherney-Craw & Lab & 714\\
Thomas Wren & LD & 94\\
Martin Malocco & Grn & 87\\
Helen Kerr & TUSC & 25\\
\end{tabular*}

\council{Three Rivers}

\subsubsection*{Gade Valley \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[12th January; LD gain from C]}

\index{Gade Valley , Three Rivers@Gade Valley, \emph{Three Rivers}}

Death of Leslie Proctor (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Alex Michaels & LD & 626\\
Dee Ward & C & 196\\
Bruce Prochnik & Lab & 119\\
David Bennett & UKIP & 69\\
Roberta Curran & Grn & 18\\
\end{tabular*}

\subsubsection*{\sloppyword{Chorleywood South and Maple Cross} \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[13th July]}

\index{Chorleywood South and Maple Cross , Three Rivers@Chorleywood S. \& Maple Cross, \emph{Three Rivers}}

Death of Ann Shaw (LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Phil Williams & LD & 1428\\
Colin Payne & C & 597\\
Jack Hazlewood & Lab & 162\\
Hazel Day & UKIP & 28\\
Tab McLaughlin & Grn & 27\\
\end{tabular*}

\subsubsection*{Oxhey Hall and Hayling \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[12th October; LD gain from C]}

\index{Oxhey Hall and Hayling , Three Rivers@Oxhey Hall \& Hayling, \emph{Three Rivers}}

Resignation of Ty Harris (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Keith Martin & LD & 672\\
Roy Clements & C & 461\\
Brendan O'Brien & Lab & 428\\
Mick Matthewson & UKIP & 35\\
Matt Jones & Grn & 31\\
\end{tabular*}

\council{Watford}

\subsubsection*{Leggatts \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Leggatts , Watford@Leggatts, \emph{Watford}}

Resignation of Anne Joynes (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Richard Smith & Lab & 934\\
Ian Stotesbury & LD & 762\\
Camilla Khawaja & C & 318\\
Su Murray & Grn & 104\\
\end{tabular*}

\subsubsection*{Oxhey \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Oxhey , Watford@Oxhey, \emph{Watford}}

Resignation of Joanna Maestas (LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Maggie Parker & LD & 1239\\
Richard Bell & C & 395\\
Sue Sleeman & Lab & 281\\
\end{tabular*}

\subsubsection*{Park \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Park , Watford@Park, \emph{Watford}}

Resignation of Anne Rindl (LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Peter Jeffree & LD & 1430\\
David Fallon & C & 915\\
Junior Sesay & Lab & 267\\
\end{tabular*}

\council{Welwyn Hatfield}

\subsubsection*{Panshanger \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Panshanger , Welwyn Hatfield@Panshanger, \emph{Welwyn Hatfield}}

Resignation of Martyn Levitt (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Terry Mitchinson & C & 643\\
Ayesha Rohale & LD & 620\\
Lucy Musk & Lab & 418\\
\end{tabular*}

\subsubsection*{Hatfield Villages \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June; Lab gain from C]}

\index{Hatfield Villages , Welwyn Hatfield@Hatfield Villages, \emph{Welwyn Hatfield}}

Death of Howard Morgan (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Tara-Mary Lyons & Lab & 1359\\
Peter Hebden & C & 1352\\
Jane Quinton & LD & 677\\
\end{tabular*}

\subsubsection*{Handside \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[14th December]}

\index{Handside , Welwyn Hatfield@Handside, \emph{Welwyn Hatfield}}

Resignation of Rachel Basch (LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Siobhan Elam & LD & 1105\\
Drew Richardson & C & 691\\
Belinda Yeldon & Lab & 260\\
Berenice Dowlen & Grn & 51\\
\end{tabular*}

\section{Isle of Wight}
\index{Isle of Wight}

At the May 2017 ordinary election there was an unfilled vacancy in Arreton and Newchurch division due to the death of Colin Richards (Ind).
\index{Arreton and Newchurch , Isle of Wight@Arreton \& Newchurch, \emph{Isle of Wight}}

\section{Kent}

\council{Ashford}

\subsubsection*{Bockhanger \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Bockhanger , Ashford@Bockhanger, \emph{Ashford}}

Resignation of Luke Powell (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Simon Howard-Smith & C & 697\\
Euan Anckorn & Lab & 415\\
Liah Macpherson & UKIP & 113\\
Steven Campkin & Grn & 97\\
\end{tabular*}

\subsubsection*{Victoria \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June; Lab gain from C]}

\index{Victoria , Ashford@Victoria, \emph{Ashford}}

Death of Harold Apps (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Charles Suddards & Lab & 1052\\
David Robey & C & 753\\
James Galloway & LD & 260\\
Serge Goldenberg & UKIP & 181\\
Maria Pizzey & Grn & 99\\
\end{tabular*}

\council{Canterbury}

\subsubsection*{Westgate \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June; Lab gain from C]}

\index{Westgate , Canterbury@Westgate, \emph{Canterbury}}

Resignation of John Brazier (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Simon Warley & Lab & 1761\\
Daniel Prevett & LD & 978\\
Luke Whiddett & C & 929\\
Henry Stanton & Grn & 389\\
\end{tabular*}

\council{Dover}

\subsubsection*{Buckland \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Buckland , Dover@Buckland, \emph{Dover}}

Resignation of Susan Hill (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Nathaniel Richards & Lab & 573\\
Callum Warriner & C & 566\\
Timothy Turner & UKIP & 192\\
\end{tabular*}

\subsubsection*{Maxton, Elms Vale and Priory \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May; Lab gain from UKIP]}

\index{Maxton, Elms Vale and Priory , Dover@Maxton, Elms Vale \& Priory, \emph{Dover}}

Resignation of Andrew Richardson (UKIP).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Ann Napier & Lab & 647\\
Roger Walkden & C & 567\\
Peter McDermott & UKIP & 159\\
Graham Wanstall & Ind & 110\\
\end{tabular*}

\subsubsection*{St Margaret's-at-Cliffe \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[23rd November]}

\index{Saint Margaret's-at-Cliffe , Dover@St Margaret's-at-Cliffe, \emph{Dover}}

Resignation of Paul Watkins (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Peter Jull & C & 750\\
Charles Woodgate & Lab & 314\\
\end{tabular*}

\council{Gravesham}

\subsubsection*{Coldharbour \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Coldharbour , Gravesham@Coldharbour, \emph{Gravesham}}

Resignation of Sue Howes (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Shane Mochrie-Cox & Lab & 490\\
Gary Harding & C & 349\\
Tina Brooker & UKIP & 268\\
\end{tabular*}

\subsubsection*{Painters Ash \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May; C gain from Lab]}

\index{Painters Ash , Gravesham@Painters Ash, \emph{Gravesham}}

Resignation of Les Howes (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Conrad Broadley & C & 873\\
Elizabeth Mulheran & Lab & 561\\
David Beattie & UKIP & 129\\
\end{tabular*}

\subsubsection*{Meopham North \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[19th October]}

\index{Meopham North , Gravesham@Meopham N., \emph{Gravesham}}

Death of John Cubitt (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Gary Harding & C & 721\\
John Death & LD & 192\\
Doug Christie & Lab & 155\\
Tina Brooker & UKIP & 59\\
\end{tabular*}

\council{Maidstone}

\subsubsection*{Bearsted \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Bearsted , Maidstone@Bearsted, \emph{Maidstone}}

Resignation of Mike Revell (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Denis Spooner & C & 1713\\
Tim Licence & Lab & 253\\
Karen Davies & LD & 250\\
Ciaran Oliver & Grn & 141\\
\end{tabular*}

\subsubsection*{North \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[30th November; LD gain from C]}

\index{North , Maidstone@North, \emph{Maidstone}}

Resignation of Michael Hemsley (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Rob Field & LD & 719\\
Cheryl Taylor-Maggio & C & 364\\
Maureen Cleator & Lab & 270\\
Derek Eagle & Grn & 47\\
\end{tabular*}

\council{Sevenoaks}

\subsubsection*{Penshurst, Fordcombe and Chiddingstone \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[3rd August]}

\index{Penshurst, Fordcombe and Chiddingstone , Sevenoaks@\sloppyword{Penshurst, Fordcombe \& Chiddingstone, \emph{Sevenoaks}}}

Death of Paddy Cooke (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Sue Coleman & C & 438\\
Richard Streatfeild & LD & 253\\
Annette Webb & Lab & 54\\
\end{tabular*}

\council{Shepway}

\subsubsection*{New Romney \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[20th July]}

\index{New Romney , Shepway@New Romney, \emph{Shepway}}

Resignation of Peter Simmons (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Russell Tillson & C & 566\\
John Cramp & Lab & 523\\
David Wimble & Ind & 431\\
Val Loseby & LD & 77\\
\end{tabular*}

\council{Swale}

\subsubsection*{Milton Regis \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[3rd August; Lab gain from UKIP]}

\index{Milton Regis , Swale@Milton Regis, \emph{Swale}}

Death of Katy Coleman (UKIP).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Tony Winckless & Lab & 573\\
Kane Blackwell & C & 255\\
Richard Palmer & UKIP & 151\\
Tony Clarke & LD & 86\\
\end{tabular*}

\council{Thanet}

\subsubsection*{Margate Central \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[3rd August; Lab gain from UKIP]}

\index{Margate Central , Thanet@Margate C., \emph{Thanet}}

Resignation of Jeffrey Elenor (Ind elected as UKIP).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Ian Venables & Lab & 454\\
Reece Pugh & C & 190\\
Liz Plewis & UKIP & 52\\
John Finnegan & LD & 33\\
Dean McCastree & Ind & 24\\
Bernard Kirkham & Grn & 23\\
Pip Rees & Ind & 13\\
\end{tabular*}

\council{Tonbridge and Malling}

\subsubsection*{\sloppyword{Aylesford North and Walderslade} \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Aylesford North and Walderslade , Tonbridge and Malling@Aylesford N. \& Walderslade, \emph{Tonbridge \& Malling}}

Resignation of Sandra Hall (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Des Keers & C & 1249\\
Mandy Morrison & LD & 349\\
Frances Long & Grn & 148\\
\end{tabular*}

\section{Lancashire}

\subsection*{County Council}\index{Lancashire}

At the May 2017 ordinary election there were unfilled vacancies in Great Harwood and Lancaster Rural North divisions due to the disqualification (non-attendance) of Gareth Molineux (Lab) and the resignation of Alycia James (C) respectively.
\index{Great Harwood , Lancashire@Great Harwood, \emph{Lancs.}}
\index{Lancaster Rural North , Lancashire@Lancaster Rural N., \emph{Lancs.}}

\council{Blackburn with Darwen}

\subsubsection*{Higher Croft \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[23rd March]}

\index{Higher Croft , Blackburn with Darwen@Higher Croft, \emph{Blackburn with Darwen}}

Death of Don McKinlay (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Adam Holden & Lab & 446\\
Ian Grimshaw & UKIP & 169\\
Maureen McGarvey & C & 133\\
\end{tabular*}

\subsubsection*{Marsh House \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May; C gain from Lab]}

\index{Marsh House , Blackburn with Darwen@Marsh House, \emph{Blackburn with Darwen}}

Death of John Roberts (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Lynn Perkins & C & 748\\
Paul Browne & LD & 580\\
Kieran Richards & Lab & 474\\
\end{tabular*}

\subsubsection*{Higher Croft \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Higher Croft , Blackburn with Darwen@Higher Croft, \emph{Blackburn with Darwen}}

\sloppyword{Disquali{fi}cation (employed by a council-owned company) of Adam Holden (Lab).}

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Matt Gibson & Lab & 1579\\
Ann Tolley & C & 818\\
Ian Grimshaw & UKIP & 343\\
\end{tabular*}

\council{Blackpool}

\subsubsection*{Warbreck \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[16th March]}

\index{Warbreck , Blackpool@Warbreck, \emph{Blackpool}}

Death of Tony Brown (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Michele Scott & C & 728\\
Ian Treasure & Lab & 468\\
Walter Cairns & UKIP & 75\\
Kevan Benfold & LD & 57\\
\end{tabular*}

\council{Fylde}

Fylde = Fylde Ratepayers

\subsubsection*{St Johns \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[9th February]}

\index{Saint Johns , Fylde@St Johns, \emph{Fylde}}

Disqualification (non-attendance) of Mark Bamforth (Fylde).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Mark Bamforth & Fylde & 564\\
Paul Lomax & C & 278\\
Jayne Boardman & Lab & 45\\
Paul Hill & Grn & 40\\
\end{tabular*}

\subsubsection*{Warton and Westby \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Warton and Westby , Fylde@Warton \& Westby, \emph{Fylde}}

Resignation of Richard Taylor (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
John Kirkham & C & 884\\
Daniel Docherty & Ind & 359\\
Jack James & Lab & 280\\
\end{tabular*}

\subsubsection*{Staining and Weeton \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[16th November]}

\index{Staining and Weeton , Fylde@Staining \& Weeton, \emph{Fylde}}

Resignation of Albert Pounder (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Jayne Nixon & C & 401\\
Nick Ansell & Lab & 111\\
Beverley Harrison & LD & 37\\
\end{tabular*}

\council{Lancaster}

\subsubsection*{Skerton West \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[7th September]}

\index{Skerton West , Lancaster@Skerton W., \emph{Lancaster}}

Resignation of Karen Leytham (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.63\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Hilda Parr & Lab & 512\\
Andy Kay & C & 288\\
Derek Kaye & LD & 33\\
\end{tabular*}

\subsubsection*{Halton-with-Aughton \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[28th September; Lab gain from Ind]}

\index{Halton-with-Aughton , Lancaster@Halton-with-Aughton, \emph{Lancaster}}

Resignation of Paul Woodruff (Ind).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Kevin Frea & Lab & 247\\
Jan Maskell & Grn & 245\\
Daniel Gibbins & C & 236\\
Catherine Pilling & LD & 174\\
\end{tabular*}

\council{Preston}

\subsubsection*{Ashton \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Ashton , Preston@Ashton, \emph{Preston}}

Resignation of Angela Vodden (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Liz Atkins & Lab & 648\\
Michael Balshaw & C & 477\\
Jeremy Dable & LD & 166\\
Simon Platt & UKIP & 62\\
\end{tabular*}

\subsubsection*{Preston Rural East \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Preston Rural East , Preston@Preston Rural E., \emph{Preston}}

Death of Tom Davies (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Ron Woollam & C & 987\\
Joshua Mascord & Lab & 216\\
David Callaghan & LD & 160\\
Kieran Aspden & UKIP & 58\\
\end{tabular*}

\council{South Ribble}

\subsubsection*{Walton-le-Dale East \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[16th March]}

\index{Walton-le-Dale East , South Ribble@Walton-le-Dale E., \emph{S. Ribble}}

Resignation of Andrea Ball (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Carol Chisholm & C & 359\\
Alex Watson & Lab & 262\\
Alison Hesketh-Holt & LD & 106\\
\end{tabular*}

\council{West Lancashire}

OWL = Our West Lancashire

\subsubsection*{Aughton Park \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Aughton Park , West Lancashire@Aughton Park, \emph{W. Lancs.}}

Resignation of Susan Bailey (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Doreen Stephenson & C & 1381\\
Bernie Green & Lab & 1095\\
\end{tabular*}

\subsubsection*{Derby \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[29th June; OWL gain from C]}

\index{Derby , West Lancashire@Derby, \emph{W. Lancs.}}

Resignation of Paul Greenall (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Ian Davis & OWL & 705\\
George Oliver & Lab & 596\\
Jane Houlgrave & C & 362\\
\end{tabular*}

\council{Wyre}

\subsubsection*{Bourne \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May; C gain from Lab]}

\index{Bourne , Wyre@Bourne, \emph{Wyre}}

Resignation of Natalie Reeves (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.6\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Emma Ellison & C & 788\\
Joanne Cooper & Lab & 621\\
Barbara Mead-Mason & Grn & 81\\
\end{tabular*}

\subsubsection*{Rossall \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[12th October]}

\index{Rossall , Wyre@Rossall, \emph{Wyre}}

Death of Ted Taylor (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Cheryl Raynor & Lab & 610\\
Maggie Pattinson & C & 427\\
Brian Crawford & Ind & 180\\
\end{tabular*}

\section{Leicestershire}

\council{Charnwood}

\subsubsection*{Birstall Wanlip \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Birstall Wanlip , Charnwood@Birstall Wanlip, \emph{Charnwood}}

Resignation of Serina Shergill (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Roy Rollings & C & 772\\
Simon Sansome & LD & 603\\
Sanjay Gogia & Lab & 425\\
Norman Cutting & Ind & 69\\
\end{tabular*}

\subsubsection*{Loughborough Shelthorpe \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[3rd August]}

\index{Loughborough Shelthorpe , Charnwood@Loughborough Shelthorpe, \emph{Charnwood}}

Resignation of Robert Sharp (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Richard Huddlestone & Lab & 595\\
Russell Ford & C & 591\\
Alex Guerrero & LD & 93\\
Andy McWilliam & UKIP & 29\\
\end{tabular*}

\subsubsection*{Loughborough Hastings (2) \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[26th October]}

\index{Loughborough Hastings , Charnwood@Loughborough Hastings, \emph{Charnwood}}

Resignations of Sarah Maynard Smith and Anne Williams (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Colin Hamilton & Lab & 676\\
Mary Draycott & Lab & 648\\
Jane Hunt & C & 228\\
Simon Murray & UKIP & 95\\
Andy McWilliam & UKIP & 79\\
Lewis Wright & Grn & 73\\
Mia Woolley & Grn & 58\\
\end{tabular*}

\council{Hinckley and Bosworth}

\subsubsection*{Burbage Sketchley and Stretton \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[5th October]}

\index{Burbage Sketchley and Streeton , Hinckley and Bosworth@Burbage Sketchley \& Stretton, \emph{Hinckley \& Bosworth}}

Resignation of Stan Rooney (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
David Macdonald & C & 822\\
Robert Mayne & LD & 785\\
Christina Emmett & Lab & 321\\
Neale Smith & UKIP & 120\\
Danny Findlay & Ind & 57\\
\end{tabular*}

\council{Leicester}

\subsubsection*{Castle \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Castle , Leicester@Castle, \emph{Leicester}}

Resignation of Lynn Senior (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Danny Myers & Lab & 4272\\
Ali Douas & C & 1015\\
Oli Young-Jones & Grn & 946\\
Lewis Hastie & LD & 379\\
\end{tabular*}

\subsubsection*{Eyres Monsell \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[23rd November]}

\index{Eyres Monsell , Leicester@Eyres Monsell, \emph{Leicester}}

Resignation of Rory Palmer MEP (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Elaine Pantling & Lab & 556\\
Tony Faithfull-Wright & LD & 320\\
Christopher Doyle & C & 170\\
\end{tabular*}

\council{Melton}

\subsubsection*{Melton Egerton \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May; C gain from Ind]}

\index{Melton Egerton , Melton@Melton Egerton, \emph{Melton}}

Resignation of Tina Culley (Ind).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Peter Faulkner & C & 356\\
Mike Brown & Lab & 320\\
\end{tabular*}

\subsubsection*{Melton Sysonby \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Melton Sysonby , Melton@Melton Sysonby, \emph{Melton}}

Death of Valerie Manderson (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Alison Freer-Jones & C & 799\\
Vanessa Jackson & Lab & 364\\
\end{tabular*}

\council{Oadby and Wigston}

\subsubsection*{Oadby Uplands \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[21st September; LD gain from Lab]}

\index{Oadby Uplands , Oadby and Wigston@Oadby Uplands, \emph{Oadby \& Wigston}}

Resignation of Gurpal Atwal (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Lily Kaufman & LD & 435\\
Matthew Luke & Lab & 384\\
Kamal Ghattoraya & C & 295\\
\end{tabular*}

\section{Lincolnshire}

\subsection*{County Council}\index{Lincolnshire}

At the May 2017 ordinary election there was an unfilled vacancy in Mablethorpe division due to the resignation of Anne Reynolds (UKIP).
\index{Mablethorpe , Lincolnshire@Mablethorpe, \emph{Lincs.}}

\council{Lincoln}

\subsubsection*{Carholme \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[19th October]}

\index{Carholme , Lincoln@Carholme, \emph{Lincoln}}

Resignation of Karen Lee MP (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.6\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Lucinda Preston & Lab & 922\\
Kateryna Roures Salvador & C & 368\\
Benjamin Loryman & Grn & 83\\
James Brown & LD & 82\\
\end{tabular*}

\columnbreak

\council{North Kesteven}

LincsInd = Lincolnshire Independents

\subsubsection*{Ashby de la Launde and Cranwell \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Ashby de la Launde and Cranwell , North Kesteven@Ashby de la Launde \& Cranwell, \emph{N. Kesteven}}

Resignation of Sarah Pearse (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.49\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Dan Gray & C & 1638\\
Pearl Clarke & LincsInd & 1152\\
\end{tabular*}

\subsubsection*{\sloppyword{Heighington and Washingborough} \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Heighington and Washingborough , North Kesteven@Heighington \& Washingborough, \emph{N. Kesteven}}

Resignation of Marion Brighton (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Edward Herring & C & 2703\\
Les Whitfield & LD & 1393\\
\end{tabular*}

\council{North Lincolnshire}

\subsubsection*{Brumby \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Brumby , North Lincolnshire@Brumby, \emph{N. Lincs.}}

Death of Pauline Carlile (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Stephen Swift & Lab & 2428\\
Janet Longcake & C & 1356\\
Dave Carswell & UKIP & 270\\
Paul Elsom & Ind & 177\\
Graham Wagstaffe & Ind & 70\\
\end{tabular*}

\council{South Holland}

\subsubsection*{\sloppyword{Whaplode and Holbeach St John's} \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[16th November]}

\index{Whaplode and Holbeach Saint John's , South Holland@Whaplode \& Holbeach St John's, \emph{S. Holland}}

Death of Mike Pullen (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Janet Whitbourn & C & 541\\
Jennie Thomas & Lab & 153\\
\end{tabular*}

\council{West Lindsey}

\subsubsection*{Scotter and Blyton \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[27th July]}

\index{Scotter and Blyton , West Lindsey@Scotter \& Blyton, \emph{W. Lindsey}}

Resignation of Adam Duguid (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Bruce Allison & C & 694\\
Liz Clews & LD & 555\\
Jonathan Harper & Lab & 230\\
Neville Jones & UKIP & 100\\
\end{tabular*}

\subsubsection*{Sudbrooke \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[16th November]}

\index{Sudbrooke , West Lindsey@Sudbrooke, \emph{W. Lindsey}}

Resignation of Stuart Curtis (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Bob Waller & C & 391\\
Gareth Hart & Lab & 171\\
\end{tabular*}

\section{Norfolk}

\subsection*{County Council}\index{Norfolk}

At the May 2017 ordinary election there was an unfilled vacancy in Lothingland division due to the death of Colin Aldred (UKIP).
\index{Lothingland , Norfolk@Lothingland, \emph{Norfolk}}

\council{Breckland}

\subsubsection*{Saham Toney \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[16th March]}

\index{Saham Toney , Breckland@Saham Toney, \emph{Breckland}}

Resignation of Charles Carter (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Theresa Hewett & C & 335\\
Andrew Thorpe & LD & 105\\
Pat Warwick & Ind & 104\\
Dave Williams & UKIP & 80\\
Joe Sisto & Lab & 72\\
\end{tabular*}

\subsubsection*{Harling and Heathlands \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Harling and Heathlands , Breckland@Harling \& Heathlands, \emph{Breckland}}

Resignation of Ellen Jolly (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Robert Kybird & C & 666\\
Christopher Harvey & Lab & 223\\
\end{tabular*}

\subsubsection*{Thetford Priory \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[28th September; Lab gain from C]}

\index{Thetford Priory , Breckland@Thetford Priory, \emph{Breckland}}

Resignation of Jane Bishop (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Mike Brindle & Lab & 503\\
Jane James & C & 257\\
Dean Roberts & UKIP & 112\\
\end{tabular*}

\council{Broadland}

\subsubsection*{Coltishall \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Coltishall , Broadland@Coltishall, \emph{Broadland}}

Resignation of Alan Mallett (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Jo Copplestone & C & 780\\
Stephen McNair & Lab & 355\\
Bob Grindrod & Ind & 288\\
James Sparshatt & LD & 154\\
\end{tabular*}

\council{King's Lynn and West Norfolk}

\subsubsection*{Fairstead \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[11th May]}

\index{Fairstead , King's Lynn and West Norfolk@Fairstead, \emph{King's Lynn \& W. Norfolk}}

Death of Ian Gourlay (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Gary Howman & Lab & 254\\
Ronald Mortimer & C & 189\\
Michael Stone & UKIP & 68\\
Rob Colwell & LD & 66\\
\end{tabular*}

\subsubsection*{St Margarets with St Nicholas \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[3rd August; C gain from Lab]}

\index{Saint Margarets with Saint Nicholas , King's Lynn and West Norfolk@St Margarets with St Nicholas, \emph{King's Lynn \& W. Norfolk}}

Disqualification (non-attendance) of Claire Kittow (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Mike Taylor & C & 253\\
Francis Bone & Lab & 210\\
Helen Russell-Johnson & LD & 173\\
Rob Archer & Grn & 63\\
\end{tabular*}

\council{North Norfolk}

\subsubsection*{Waterside \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[9th February; LD gain from C]}

\index{Waterside , North Norfolk@Waterside, \emph{N. Norfolk}}

\sloppyword{Disquali{fi}cation (non-attendance) of Ben Jarvis (C).}

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Marion Millership & LD & 649\\
Tony Lumbard & C & 410\\
Barry Whitehouse & UKIP & 77\\
David Russell & Lab & 41\\
\end{tabular*}

\subsubsection*{Holt \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May; C gain from LD]}

\index{Holt , North Norfolk@Holt, \emph{N. Norfolk}}

Resignation of Philip High (LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Duncan Baker & C & 724\\
Naomi Farrow & LD & 546\\
Richard Kelham & Lab & 90\\
Terence Comber & UKIP & 84\\
\end{tabular*}

\section{North Yorkshire}

Yorks = Yorkshire Party

\subsection*{County Council}\index{North Yorkshire}

At the May 2017 ordinary election there was an unfilled vacancy in Richmondshire North division due to the death of Michael Heseltine (C).
\index{Richmondshire North , North Yorkshire@Richmondshire N., \emph{N. Yorks.}}

\council{Craven}

\subsubsection*{Aire Valley with Lothersdale \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May; Grn gain from C]}

\index{Aire Valley with Lothersdale , Craven@Aire Valley with Lothersdale, \emph{Craven}}

Resignation of Patricia Fairbank (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Andrew Brown & Grn & 652\\
Gemma Harling & C & 644\\
\end{tabular*}

\council{Harrogate}

\subsubsection*{Washburn \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[28th September]}

\index{Washburn , Harrogate@Washburn, \emph{Harrogate}}

Resignation of Christine Ryder (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Victoria Oldham & C & 363\\
Laura Dinning & Lab & 61\\
Paul Trewhitt & Grn & 44\\
Jack Render & Yorks & 19\\
\end{tabular*}

\council{Middlesbrough}

\subsubsection*{Coulby Newham \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[13th April; C gain from Lab]}

\index{Coulby Newham , Middlesbrough@Coulby Newham, \emph{Middlesbrough}}

Resignation of Geoff Cole (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Jacob Young & C & 501\\
Annalise Higgins & Lab & 468\\
Mike Allen & Ind & 318\\
Emma Alberti & Grn & 32\\
\end{tabular*}

\subsubsection*{Ayresome \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[13th July]}

\index{Ayresome , Middlesbrough@Ayresome, \emph{Middlesbrough}}

Death of Bernie Taylor (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Vic Walkington & Lab & 414\\
Jill Coleman & C & 252\\
Rhid Nugent & LD & 15\\
Carl Martinez & Grn & 13\\
\end{tabular*}

\subsubsection*{Park End and Beckfield \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[13th July]}

\index{Park End and Beckfield , Middlesbrough@Park End \& Beckfield, \emph{Middlesbrough}}

Death of Peter Cox (Ind).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Jan Mohan & Ind & 505\\
Ian Blades & Lab & 302\\
Ron Armstrong & C & 59\\
Jamie Rawlings & Grn & 12\\
Wen Cai Bowman & LD & 10\\
\end{tabular*}

\council{Redcar and Cleveland}

\subsubsection*{Hutton \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[2nd March]}

\index{Hutton , Redcar and Cleveland@Hutton, \emph{Redcar \& Cleveland}}

Resignation of Valerie Halton (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Alma Thrower & C & 860\\
Graeme Kidd & LD & 326\\
Ian Urwin & Lab & 183\\
Barry Hudson & UKIP & 129\\
\end{tabular*}

\subsubsection*{Newcomen \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[2nd March]}

\index{Newcomen , Redcar and Cleveland@Newcomen, \emph{Redcar \& Cleveland}}

Death of Chris Abbott (LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Laura Benson & LD & 426\\
Charlie Brady & Lab & 259\\
Andrea Turner & UKIP & 153\\
Mark Hannon & Ind & 52\\
Dave Stones & Ind & 36\\
Maret Ward & C & 29\\
\end{tabular*}

\subsubsection*{St Germain's \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[5th October]}

\index{Saint Germain's , Redcar and Cleveland@St Germain's, \emph{Redcar \& Cleveland}}

Death of Marjorie Moses (LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Deborah Dowson & LD & 661\\
David Jones & Lab & 368\\
John Lambert & Ind & 261\\
Vic Jeffries & Ind & 225\\
Olwyn Twentyman & C & 174\\
Nicola Riley & Grn & 31\\
\end{tabular*}

\council{Richmondshire}

\subsubsection*{Reeth and Arkengarthdale \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[18th May; C gain from Ind]}

\index{Reeth and Arkengarthdale , Richmondshire@Reeth \& Arkengarthdale, \emph{Richmondshire}}

Death of Richard Beal (Ind).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Ian Scott & C & \emph{unop.}\\
\end{tabular*}

\council{Scarborough}

\subsubsection*{Mulgrave \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[Tuesday 29th August]}

\index{Mulgrave , Scarborough@Mulgrave, \emph{Scarborough}}

Resignation of Marie Harland (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Marion Watson & C & 395\\
Hugo Fearnley & Lab & 299\\
John Armsby & Ind & 118\\
Lee Derrick & Yorks & 37\\
\end{tabular*}

\council{York}

\subsubsection*{Hull Road \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Hull Road , York@Hull Rd., \emph{York}}

Resignation of David Levene (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Michael Pavlovic & Lab & 3408\\
Matt Freckelton & C & 1199\\
John Cossham & Grn & 826\\
Alex Nowis & LD & 407\\
\end{tabular*}

\subsubsection*{Micklegate \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Micklegate , York@Micklegate, \emph{York}}

Resignation of Julie Gunnell (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Jonny Crawshaw & Lab & 3772\\
Rosie Baker & Grn & 1581\\
Paul Healey & C & 1460\\
Aileen Hingston & LD & 634\\
\end{tabular*}

\section{Northamptonshire}

\council{Corby}

\subsubsection*{Kingswood and Hazel Leys \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[9th February]}

\index{Kingswood and Hazel Leys , Corby@Kingswood \& Hazel Leys, \emph{Corby}}

Disqualification (non-attendance) of Kenneth Carratt (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Isabel McNab & Lab & 610\\
Stan Heggs & C & 252\\
Michael Mahon & Grn & 82\\
\end{tabular*}

\council{Daventry}

\subsubsection*{Long Buckby \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Long Buckby , Daventry@Long Buckby, \emph{Daventry}}

Resignation of Diana Osborne (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Malcolm Longley & C & 1170\\
Sue Myers & Lab & 565\\
Neil Farmer & LD & 253\\
Tom Harper & Grn & 169\\
Ian Dexter & UKIP & 151\\
\end{tabular*}

\council{East Northamptonshire}

\subsubsection*{Prebendal \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Prebendal , East Northamptonshire@Prebendal, \emph{E. Northants.}}

Resignation of Valerie Raven-Hill (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.6\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Annabel de Capell Brooke & C & 729\\
Alan Brookfield & Lab & 151\\
\end{tabular*}

\council{Kettering}

\subsubsection*{Barton \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[23rd February; LD gain from C]}

\index{Barton , Kettering@Barton, \emph{Kettering}}

Resignation of Stephen Bellamy (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Andrew Dutton & LD & 644\\
Dianne Miles-Zanger & C & 337\\
Robert Clements & UKIP & 106\\
Rob Reeves & Grn & 42\\
\end{tabular*}

\subsubsection*{Burton Latimer \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Burton Latimer , Kettering@Burton Latimer, \emph{Kettering}}

Resignation of Derek Zanger (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
John Currall & C & 641\\
Christopher Groome & Ind & 531\\
Sam Watts & UKIP & 269\\
Samuel Nicholls & Lab & 264\\
Jenny Davies & LD & 189\\
April Wright & Grn & 40\\
\end{tabular*}

\subsubsection*{St Peter's \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Saint Peter's , Kettering@St Peter's, \emph{Kettering}}

Resignation of Mary Malin (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Paul Marks & C & 774\\
Eugene Dalton-Ruark & Lab & 323\\
Chris McGlynn & LD & 98\\
Geri Coop & Grn & 93\\
Stewart Farrant & UKIP & 63\\
\end{tabular*}

\council{Northampton}

\subsubsection*{Eastfield \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[28th September]}

\index{Eastfield , Northampton@Eastfield, \emph{Northampton}}

Resignation of Elizabeth Gowen (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Paul Joyce & Lab & 493\\
Pauline Woodhouse & C & 288\\
Martin Sawyer & LD & 195\\
\end{tabular*}

\subsubsection*{Nene Valley \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[28th September]}

\index{Nene Valley , Northampton@Nene Valley, \emph{Northampton}}

Resignation of Michael Hill (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Luke Graystone & C & 803\\
Nikesh Jani & Lab & 343\\
Brian Hoare & LD & 293\\
Denise Donaldson & Grn & 91\\
\end{tabular*}

\council{South Northamptonshire}

\subsubsection*{Kings Sutton \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Kings Sutton , South Northamptonshire@Kings Sutton, \emph{S. Northants.}}

Resignation of Ian Morris (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Gregor Hopkins & C & 470\\
Michael Bailey & Lab & 232\\
\end{tabular*}

\section{Northumberland}

At the May 2017 ordinary election there was an unfilled vacancy in Lynemouth division due to the death of Milburn Douglas (Lab).
\index{Lynemouth , Northumberland@Lynemouth, \emph{Northd.}}

\section{Nottinghamshire}

\council{Ashfield}

Ashfield = Ashfield Independent

\subsubsection*{Hucknall North \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[12th October; Ashfield gain from C]}

\index{Hucknall North , Ashfield@Hucknall N., \emph{Ashfield}}

Resignation of Ben Bradley MP (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.5\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
John Wilmott & Ashfield & 1329\\
Ian Morrison & Lab & 629\\
Sheila Clarke & C & 532\\
Stephen Crosby & UKIP & 66\\
James Harvey & LD & 46\\
\end{tabular*}

\council{Broxtowe}

\subsubsection*{Toton and Chilwell Meadows \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Toton and Chilwell Meadows , Broxtowe@Toton \& Chilwell Meadows, \emph{Broxtowe}}

Resignation of Mia Kee (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Lee Fletcher & C & 2471\\
David Patrick & Lab & 1573\\
Graham Heal & LD & 624\\
Gordon Stoner & Grn & 180\\
Chris Cobb & UKIP & 163\\
\end{tabular*}

\council{Newark and Sherwood}

\subsubsection*{Ollerton \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Ollerton , Newark and Sherwood@Ollerton, \emph{Newark \& Sherwood}}

Resignation of Derek Batey (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Neal Mitchell & Lab & 1469\\
Glenn Bardill & C & 913\\
\end{tabular*}

\columnbreak

\council{Nottingham}

Elvis = Elvis and the Yeti Himalayan Preservation (Sherwood, 4th May); Bus-Pass Elvis (Bestwood, 19th October)

\subsubsection*{Sherwood \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Sherwood , Nottingham@Sherwood, \emph{Nottingham}}

Resignation of Alex Ball (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Adele Williams & Lab & 2816\\
William Scott & C & 847\\
Barry Holliday & LD & 359\\
Zeb Brigham & Grn & 221\\
Carol Boultby & UKIP & 139\\
David Bishop & Elvis & 51\\
\end{tabular*}

\subsubsection*{Basford \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[19th October]}

\index{Basford , Nottingham@Basford, \emph{Nottingham}}

Resignation of Alex Norris MP (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Nick Raine & Lab & 1409\\
Bradley Wing & C & 408\\
Bill Ottewell & UKIP & 119\\
Zeb Brigham & Grn & 81\\
Rebecca Proctor & LD & 49\\
\end{tabular*}

\subsubsection*{Bestwood \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[19th October]}

\index{Bestwood , Nottingham@Bestwood, \emph{Nottingham}}

Resignation of Mick Wildgust (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Georgia Power & Lab & 1280\\
Francesco Lari & UKIP & 301\\
William Scott & C & 297\\
\sloppyword{Christina Morgan-Danvers} & LD & 57\\
Liam McClelland & Grn & 50\\
David Bishop & Elvis & 34\\
\end{tabular*}

\subsubsection*{Bulwell Forest \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[19th October]}

\index{Bulwell Forest , Nottingham@Bulwell Forest, \emph{Nottingham}}

Death of Alan Clark (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Cheryl Barnard & Lab & 1420\\
Karen Kemp & C & 966\\
Tony Blay & UKIP & 141\\
Andrew Jones & Grn & 52\\
Callum Southern & LD & 31\\
\end{tabular*}

\council{Rushcliffe}

\subsubsection*{Thoroton \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Thoroton , Rushcliffe@Thoroton, \emph{Rushcliffe}}

Resignation of Adeline Pell (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Sarah Bailey & C & 615\\
Tracey Kerry & Ind & 215\\
David Allen & LD & 125\\
\end{tabular*}

\section{Oxfordshire}

\council{Oxford}

\subsubsection*{Barton and Sandhills \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Barton and Sandhills , Oxford@Barton \& Sandhills, \emph{Oxford}}

Death of Van Coulter (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Mark Ladbrooke & Lab & 698\\
Peter Saville & C & 284\\
Chaka Artwell & Ind & 203\\
Steve Wheeler & LD & 194\\
Symon Hill & Grn & 103\\
\end{tabular*}

\council{South Oxfordshire}

\subsubsection*{Didcot South \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[13th July]}

\index{Didcot South , South Oxfordshire@Didcot S., \emph{S. Oxon.}}

Resignation of Margaret Davies (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Mocky Khan & Lab & 631\\
Jackie Billington & C & 528\\
Veronika Williams & LD & 289\\
\end{tabular*}

\subsubsection*{Didcot West \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[13th July]}

\index{Didcot West , South Oxfordshire@Didcot W., \emph{S. Oxon.}}

Resignation of Margaret Turner (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Ian Snowdon & C & 429\\
Denise Macdonald & Lab & 393\\
Ian Smith & LD & 172\\
\end{tabular*}

\subsubsection*{Haseley Brook \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[19th October]}

\index{Haseley Brook , South Oxfordshire@Haseley Brook, \emph{S. Oxon.}}

Resignation of Stephen Harrod (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Caroline Newton & C & \emph{unop.}\\
\end{tabular*}

\council{West Oxfordshire}

\subsubsection*{Hailey, Minster Lovell and Leafield \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[9th March; LD gain from C]}

\index{Hailey, Minster Lovell and Leafield , West Oxfordshire@Hailey, Minster Lovell \& Leafield, \emph{W. Oxon.}}

Death of Warwick Robinson (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Kieran Mullins & LD & 567\\
Brendan Kay & C & 504\\
Calvert McGibbon & Lab & 71\\
Andrew Wright & Grn & 38\\
Jim Stanley & UKIP & 35\\
\end{tabular*}

\subsubsection*{The Bartons \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Bartons , West Oxfordshire@The Bartons, \emph{W. Oxon.}}

Resignation of Robert Courts MP (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Charles Virgin & C & 431\\
Mark Mann & LD & 145\\
Dave Baldwin & Lab & 54\\
\end{tabular*}

\section{Rutland}\index{Rutland}

\subsubsection*{Exton \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[9th March]}

\index{Exton , Rutland@Exton, \emph{Rutland}}

Resignation of Terry King (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
June Fox & C & 238\\
Joanna Burrows & LD & 123\\
Claire Barks & UKIP & 39\\
\end{tabular*}

\subsubsection*{Ketton \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[20th July]}

\index{Ketton , Rutland@Ketton, \emph{Rutland}}

Resignation of Diana MacDuff (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Gordon Brown & C & 459\\
Kenneth Siddle & LD & 208\\
\end{tabular*}

\subsubsection*{Whissendine \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[20th July; Ind gain from LD]}

\index{Whissendine , Rutland@Whissendine, \emph{Rutland}}

Resignation of Kevin Thomas (LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Ian Arnold & Ind & 212\\
Peter Jones & C & 102\\
Sue Lammin & Ind & 46\\
Johannah Randall & LD & 32\\
\end{tabular*}

\section{Somerset}

\council{Bath and North East Somerset}

\subsubsection*{Walcot \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[6th April]}

\index{Walcot , Bath and North East Somerset@Walcot, \emph{Bath \& N.E. Somerset}}

Resignation of Lisa Brett (LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Richard Samuel & LD & 750\\
Tim Stoneman & Grn & 343\\
Brian Webber & C & 339\\
Amber Weston & Lab & 111\\
\end{tabular*}

\subsubsection*{Newbridge \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May; LD gain from C]}

\index{Newbridge , Bath and North East Somerset@Newbridge, \emph{Bath \& N.E. Somerset}}

Resignation of Donal Hassett (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.63\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Michelle O'Doherty & LD & 828\\
Rob Hawking & C & 783\\
Loraine Morgan-Brinkhurst & Ind & 242\\
Dick Daniel & Grn & 209\\
Pam Richards & Lab & 174\\
\end{tabular*}

\council{Mendip}

\subsubsection*{Frome College \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Frome College , Mendip@Frome College, \emph{Mendip}}

Resignation of Mike Rideout (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Carole Bullen & C & 576\\
Alex Shingler & LD & 469\\
Pepita Collins & Grn & 192\\
Derek & Lib & 64\\
\end{tabular*}

\council{North Somerset}

\subsubsection*{\sloppyword{Weston-super-Mare North Worle} \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[31st August; Lab gain from North Somerset First Independents]}

\index{Weston-super-Mare North Worle , North Somerset@\sloppyword{Weston-super-Mare North Worle, \emph{N. Somerset}}}

Death of Derek Mead (North Somerset First Independents).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Denise Hunt & Lab & 589\\
James Davis & C & 525\\
Alan Rice & LD & 265\\
Richard Skinner & Ind & 132\\
Anita Spencer-Johns & UKIP & 108\\
\end{tabular*}

\council{Sedgemoor}

\subsubsection*{Bridgwater Eastover \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Bridgwater Eastover , Sedgemoor@Bridgwater Eastover, \emph{Sedgemoor}}

Resignation of Moira Brown (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Chelsea Chadwick & Lab & 287\\
Pele Barnes & C & 256\\
Antony Nickolls & LD & 132\\
Dave Preece & Ind & 60\\
\end{tabular*}

\council{South Somerset}

\subsubsection*{Blackmoor Vale \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Blackmoor Vale , South Somerset@Blackmoor Vale, \emph{S. Somerset}}

Death of Tim Inglefield (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Hayward Burt & C & 1432\\
Grahame Baker & LD & 395\\
Oliver Dowding & Grn & 205\\
\end{tabular*}

\council{West Somerset}

\subsubsection*{Dunster and Timberscombe \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[23rd March; LD gain from C]}

\index{Dunster and Timberscombe , West Somerset@Dunster \& Timberscombe, \emph{W. Somerset}}

Resignation of Bryan Leaker (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Peter Pilkington & LD & 174\\
Andy Parbrook & C & 115\\
Lucy McQuillan & Grn & 38\\
Maureen Smith & Lab & 23\\
\end{tabular*}

\subsubsection*{Minehead North \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Minehead North , West Somerset@Minehead N., \emph{W. Somerset}}

Resignation of David Archer (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.63\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Andrew Kingston-James & C & 410\\
Benet Allen & LD & 217\\
Maureen Smith & Lab & 104\\
Sally Kuhlmann Hall & Ind & 67\\
\end{tabular*}

\section{Staffordshire}

ChaseInd = The Chase Independent Party

\subsection*{County Council}\index{Staffordshire}

\subsubsection*{Hednesford and Rawnsley \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[7th September]}

\index{Hednesford and Rawnsley , Staffordshire@Hednesford \& Rawnsley, \emph{Staffs.}}

Death of Graham Burnett (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.495\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Bryan Jones & C & 1484\\
George Adamson & Lab & 1454\\
Paul Woodhead & Grn & 1316\\
John Bernard & UKIP & 175\\
Pat Ansell & LD & 67\\
Ron Turville & ChaseInd & 65\\
\end{tabular*}

\council{Cannock Chase}

\subsubsection*{Hednesford Green Heath \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[7th September; Lab gain from C]}

\index{Hednesford Green Heath , Cannock Chase@\sloppyword{Hednesford Green Heath, \emph{Cannock Chase}}}

Death of Graham Burnett (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.52\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Linda Tait & Lab & 359\\
Bryan Jones & C & 301\\
Robert Branson & Grn & 86\\
Ron Turville & ChaseInd & 42\\
David Percox & UKIP & 29\\
\end{tabular*}

\subsubsection*{Hednesford South \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[7th September; Grn gain from C]}

\index{Hednesford South , Cannock Chase@Hednesford S., \emph{Cannock Chase}}

Disqualification (non-attendance) of Joanne Christian (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Stuart Crabtree & Grn & 513\\
Philip Hewitt & C & 311\\
Patrick Frondigoun & Lab & 190\\
John Bernard & UKIP & 48\\
\end{tabular*}

\council{East Staffordshire}

\subsubsection*{Town \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[2nd February]}

\index{Town , East Staffordshire@Town, \emph{E. Staffs.}}

Resignation of Karen Haberfield (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Philip Hudson & C & 627\\
Zdzislaw Krupski & Lab & 359\\
Norman Moir & UKIP & 213\\
\end{tabular*}

\subsubsection*{Burton \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[16th February]}

\index{Burton , East Staffordshire@Burton, \emph{E. Staffs.}}

Resignation of Michael Rodgers (LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Helen Hall & LD & 271\\
Phil Hutchinson & Lab & 127\\
Peter Levis & UKIP & 60\\
Hamid Asghar & C & 56\\
\end{tabular*}

\subsubsection*{Horninglow \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May; C gain from Lab]}

\index{Horninglow , East Staffordshire@Horninglow, \emph{E. Staffs.}}

Resignation of Julian Mott (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.534\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Deneice Florence-Jukes & C & 800\\
Louise Walker & Lab & 756\\
Peter Levis & UKIP & 199\\
Hugh Warner & LD & 53\\
\end{tabular*}

\subsubsection*{Shobnall \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Shobnall , East Staffordshire@Shobnall, \emph{E. Staffs.}}

Resignation of Bill Ganley (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Paul Walker & Lab & 794\\
Colin Wood & C & 504\\
Mike Green & UKIP & 113\\
Monica Douglas-Clark & LD & 96\\
Simon Hales & Grn & 54\\
\end{tabular*}

\subsubsection*{Stretton \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[28th September]}

\index{Stretton , East Staffordshire@Stretton, \emph{E. Staffs.}}

Resignation of Rebecca Carlton (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Dale Spedding & C & 762\\
Graham Lamb & Ind & 455\\
Cameron McKiernan & Lab & 311\\
Peter Levis & UKIP & 52\\
Rhys Buchan & LD & 36\\
\end{tabular*}

\council{Lichfield}

\subsubsection*{Fazeley \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May; C gain from UKIP]}

\index{Fazeley , Lichfield@Fazeley, \emph{Lichfield}}

Resignation of John Mills (UKIP).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Brian Hoult & C & 652\\
Glen Mynott & Lab & 343\\
Edwin Jones & Grn & 66\\
\end{tabular*}

\council{Newcastle-under-Lyme}

\subsubsection*{Bradwell \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[14th December]}

\index{Bradwell , Newcastle-under-Lyme@Bradwell, \emph{Newcastle-under-Lyme}}

Death of Trevor Hambleton (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Andrew Fox-Hewitt & Lab & 396\\
Gail Benbow & C & 360\\
Richard Virr & LD & 25\\
\end{tabular*}

\subsubsection*{Newchapel \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[14th December; C gain from Lab]}

\index{Newchapel , Newcastle-under-Lyme@\sloppyword{Newchapel, \emph{Newcastle-under-Lyme}}}

Resignation of Lionel Burch (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Jill Waring & C & 216\\
Angela Cooper & Lab & 197\\
Nigel Morgan & Ind & 86\\
\end{tabular*}

\council{South Staffordshire}

\subsubsection*{Penkridge West \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Penkridge West , South Staffordshire@Penkridge W., \emph{S. Staffs.}}

Resignation of Don Cartwright (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Josephine Chapman & C & 357\\
Andrew Lenz & Lab & 101\\
David Oldfield & Ind & 99\\
Victor Kelly & Ind & 39\\
Ian Sadler & Grn & 15\\
Norman Smallwood & Ind & 11\\
\end{tabular*}

\council{Stafford}

\subsubsection*{Highfields and Western Downs (2) \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May; 2 C gains from Lab]}

\index{Highfields and Western Downs , Stafford@Highfields \& Western Downs, \emph{Stafford}}

Resignations of Maureen Bowen and Stephen O'Connor (both Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Victoria Jenkinson & C & 650\\
William Taylor & C & 620\\
David Barron & Lab & 602\\
Mike Winkle & Lab & 544\\
Toby Hollinshead & Grn & 109\\
Doug Rouxel & Grn & 97\\
\end{tabular*}

\council{Staffordshire Moorlands}

\subsubsection*{Leek East \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[20th July; Lab gain from C]}

\index{Leek East , Staffordshire Moorlands@Leek E., \emph{Staffs. Moorlands}}

Resignation of Rebecca Done (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Darren Price & Lab & 505\\
Roy Tomkinson & C & 325\\
Nick Sheldon & Ind & 219\\
Roy Gregg & LD & 74\\
\end{tabular*}

\council{Tamworth}

\subsubsection*{Bolehall \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[12th October]}

\index{Bolehall , Tamworth@Bolehall, \emph{Tamworth}}

Resignation of Peter Seekings (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Sheree Peaple & Lab & 643\\
Thomas Jay & C & 561\\
\end{tabular*}

\section{Suffolk}

\subsection*{County Council}\index{Suffolk}

At the May 2017 ordinary election there was an unfilled vacancy in Lowestoft South division due to the resignation of Derek Hackett (UKIP).
\index{Lowestoft South , Suffolk@Lowestoft S., \emph{Suffolk}}

\subsubsection*{St John's \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[7th September]}

\index{Saint John's , Suffolk@St John's, \emph{Suffolk}}

Resignation of Sandy Martin MP (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Sarah Adams & Lab & 1247\\
James Harding & C & 483\\
Edward Packard & LD & 200\\
Charlotte Armstrong & Grn & 52\\
\end{tabular*}

\council{Babergh}

\subsubsection*{Sudbury South \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[7th September; Lab gain from C]}

\index{Sudbury South , Babergh@Sudbury S., \emph{Babergh}}

Resignation of David Holland (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.63\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Luke Cresswell & Lab & 336\\
Simon Sudbury & C & 335\\
Andrew Welsh & LD & 116\\
\end{tabular*}

\council{Forest Heath}

\subsubsection*{St Mary's \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[17th August]}

\index{Saint Mary's , Forest Heath@St Mary's, \emph{Forest Heath}}

Death of Bill Sadler (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Robert Nobbs & C & 338\\
Michael Jefferys & Lab & 276\\
Alice Haylock & Grn & 60\\
\end{tabular*}

\council{Ipswich}

\subsubsection*{Stoke Park \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Stoke Park , Ipswich@Stoke Park, \emph{Ipswich}}

Resignation of Robin Hyde-Chambers (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Bob Hall & C & 899\\
Collette Allen & Lab & 659\\
Alan Cotterell & UKIP & 143\\
Maureen Haaker & LD & 59\\
Adria Pittock & Grn & 53\\
\end{tabular*}

\council{Mid Suffolk}

\subsubsection*{Eye \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May; C gain from Ind]}

\index{Eye , Mid Suffolk@Eye, \emph{Mid Suffolk}}

Resignation of Charles Flatman (Ind).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Michael Burke & C & 438\\
Simon Hooton & Grn & 220\\
John Blake & LD & 112\\
Alex Wilson & Lab & 68\\
\end{tabular*}

\subsection*{St Edmundsbury}\index{Saint Edmundsbury@St Edmundsbury}

\subsubsection*{Chedburgh \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[28th September]}

\index{Chedburgh , Saint Edmundsbury@Chedburgh, \emph{St Edmundsbury}}

Death of Angela Rushen (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Mike Chester & C & 372\\
Gary Dillon & Lab & 128\\
Ian Chapman & LD & 65\\
\end{tabular*}

\subsubsection*{Hundon \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[28th September]}

\index{Hundon , Saint Edmundsbury@Hundon, \emph{St Edmundsbury}}

Resignation of Jeremy Farthing (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Mary Evans & C & 357\\
Alex Rolph & LD & 86\\
\end{tabular*}

\council{Waveney}

\subsubsection*{Oulton Broad \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[21st September]}

\index{Oulton Broad , Waveney@Oulton Broad, \emph{Waveney}}

Death of Colin Law (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Keith Robinson & C & 527\\
Len Jacklin & Lab & 357\\
Phillip Trindall & UKIP & 112\\
Chris Thomas & LD & 54\\
\end{tabular*}

\subsubsection*{Kirkley \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[16th November]}

\index{Kirkley , Waveney@Kirkley, \emph{Waveney}}

Resignation of Steve Logan (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Peter Byatt & Lab & 374\\
Gilly Gunner & C & 217\\
Dominic Leslie & LD & 84\\
Phillip Trindall & UKIP & 78\\
Ben Quail & Grn & 30\\
\end{tabular*}

\subsubsection*{St Margaret's \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[16th November; C gain from Lab]}

\index{Saint Margaret's , Waveney@St Margaret's, \emph{Waveney}}

Resignation of Louisa Harris-Logan (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Linda Coulam & C & 487\\
Nasima Begum & Lab & 410\\
Bernie Guymer & UKIP & 119\\
Shaun Waters & LD & 88\\
Baz Bemment & Grn & 65\\
\end{tabular*}

\section{Surrey}

\council{Reigate and Banstead}

\subsubsection*{Tadworth and Walton \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Tadworth and Walton , Reigate and Banstead@Tadworth \& Walton, \emph{Reigate \& Banstead}}

Resignation of James Clarke (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
George Curry & C & 1590\\
Margaret Walkling & Ind & 394\\
Valerie Moore & UKIP & 148\\
\end{tabular*}

\council{Runnymede}

\subsubsection*{Foxhills \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Foxhills , Runnymede@Foxhills, \emph{Runnymede}}

Resignation of Howard Butterfield (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Emma Bancroft & C & 893\\
Don Whyte & LD & 243\\
Peter Kingham & Lab & 188\\
Valerie Woodhouse & UKIP & 173\\
Lee-Anne Lawrence & Grn & 49\\
\end{tabular*}

\council{Spelthorne}

\subsubsection*{Ashford East \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Ashford East , Spelthorne@Ashford E., \emph{Spelthorne}}

Resignation of Chris Frazer (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Alex Sapunovas & C & 951\\
Paul West & UKIP & 384\\
Mark Appleyard & Lab & 382\\
Thomas Fidler & LD & 185\\
\end{tabular*}

\council{Tandridge}

\subsubsection*{Westway \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[30th November]}

\index{Westway , Tandridge@Westway, \emph{Tandridge}}

Resignation of Caroline Warner (LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Helen Rujbally & LD & 483\\
Alex Standen & C & 239\\
Lucy McNally & Lab & 118\\
Helena Windsor & UKIP & 62\\
\end{tabular*}

\columnbreak

\council{Waverley}

\subsubsection*{Godalming Central and Ockford \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[Wednesday 13th December; LD gain from C]}

\index{Godalming Central and Ockford , Waverley@Godalming C. \& Ockford, \emph{Waverley}}

Resignation of Simon Thornton (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Paul Follows & LD & 266\\
Stella Andersen-Payne & C & 246\\
Richard Ashworth & Lab & 151\\
Susan Ryland & Grn & 40\\
\end{tabular*}

\section{Warwickshire}

\council{North Warwickshire}

\subsubsection*{Coleshill South \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[13th July]}

\index{Coleshill South , North Warwickshire@Coleshill S., \emph{N. Warks.}}

Resignation of Mark Jones (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Caroline Symonds & C & 571\\
Claire Breeze & Lab & 379\\
\end{tabular*}

\council{Nuneaton and Bedworth}

\subsubsection*{Bulkington \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May; C gain from Lab]}

\index{Bulkington , Nuneaton and Bedworth@Bulkington, \emph{Nuneaton \& Bedworth}}

Resignation of Terry Doherty (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Rich Smith & C & 1651\\
Jack Bonner & Lab & 517\\
\end{tabular*}

\council{Rugby}

\subsubsection*{Coton and Boughton \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Coton and Boughton , Rugby@Coton \& Boughton, \emph{Rugby}}

Resignation of Helen Taylor (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Sebastian Lowe & C & 809\\
Alan Webb & Lab & 533\\
Jerome Perrier & LD & 120\\
Peter Reynolds & Grn & 89\\
\end{tabular*}

\subsubsection*{New Bilton \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{New Bilton , Rugby@New Bilton, \emph{Rugby}}

Resignation of Steve Birkett (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Mike Brader & Lab & 784\\
Zoe Feeney & C & 505\\
Roy Sandison & Grn & 107\\
Hossain Tafazzal & LD & 76\\
\end{tabular*}

\council{Stratford-on-Avon}

\subsubsection*{Red Horse \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[9th March]}

\index{Red Horse , Stratford-on-Avon@Red Horse, \emph{Stratford-on-Avon}}

Resignation of Bart Mura (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
John Feilding & C & 476\\
Philip Vial & LD & 266\\
Edward Fila & UKIP & 92\\
Pat Hotson & Grn & 58\\
\end{tabular*}

\subsubsection*{Ettington \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Ettington , Stratford-on-Avon@Ettington, \emph{Stratford-on-Avon}}

Resignation of Philip Seccombe (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.62\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Penny-Anne O'Donnell & C & 949\\
Bill Dowling & LD & 227\\
Dave Passingham & Grn & 102\\
\end{tabular*}

\council{Warwick}

\subsubsection*{Stoneleigh and Cubbington \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[5th October]}

\index{Stoneleigh and Cubbington , Warwick@\sloppyword{Stoneleigh \& Cubbington, \emph{Warwick}}}

Resignation of Nick Harrington (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Trevor Wright & C & 502\\
Josh Payne & Lab & 311\\
Richard Dickson & LD & 113\\
Chris Philpott & Grn & 29\\
\end{tabular*}

\section{West Sussex}

\subsection*{County Council}\index{West Sussex}

At the May 2017 ordinary election there were unfilled vacancies in East Grinstead South and Ashurst Wood, and Roffey divisions due to the resignations of John O'Brien and Jim Rae (both C) respectively.
\index{East Grinstead South and Ashurst Wood , West Sussex@East Grinstead S. \& Ashurst Wood, \emph{W. Sussex}}
\index{Roffey , West Sussex@Roffey, \emph{W. Sussex}}

\council{Adur}

\subsubsection*{Mash Barn \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[5th October; Lab gain from UKIP]}

\index{Mash Barn , Adur@Mash Barn, \emph{Adur}}

Resignation of David Lambourne (UKIP).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Lee Cowen & Lab & 490\\
Jack Howard & C & 384\\
Doris Martin & LD & 89\\
Paul Hendy & Grn & 31\\
\end{tabular*}

\council{Arun}

\subsubsection*{Aldwick West \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[2nd November; LD gain from C]}

\index{Aldwick West , Arun@Aldwick W., \emph{Arun}}

Death of Jacqui Maconachie (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Martin Smith & LD & 719\\
Guy Purser & C & 480\\
Ian Manion & Lab & 112\\
Carol Birch & Grn & 54\\
\end{tabular*}

\council{Chichester}

\subsubsection*{Bosham \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Bosham , Chichester@Bosham, \emph{Chichester}}

Resignation of Myles Cullen (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Andy Collins & C & 859\\
Adrian Moss & LD & 649\\
\end{tabular*}

\subsubsection*{East Wittering \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{East Wittering , Chichester@East Wittering, \emph{Chichester}}

Resignation of Ian Curbishley (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Keith Martin & C & 965\\
Joe O'Sullivan & Lab & 177\\
Jane Scotland & LD & 169\\
Andrew Emerson & Patria & 18\\
\end{tabular*}

\subsubsection*{North Mundham \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{North Mundham , Chichester@North Mundham, \emph{Chichester}}

Resignation of Paul Jarvis (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Chris Page & C & 471\\
Robert Linzell & LD & 221\\
\end{tabular*}

\subsubsection*{Plaistow \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Plaistow , Chichester@Plaistow, \emph{Chichester}}

Resignation of Denise Knightley (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Peter Wilding & C & 1042\\
Raymond Cooper & LD & 373\\
\end{tabular*}

\council{Crawley}

\subsubsection*{Maidenbower \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Maidenbower , Crawley@Maidenbower, \emph{Crawley}}

Resignation of Ken Trussell (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Nigel Boxall & C & 1377\\
Morgan Flack & Lab & 413\\
Paul Cummings & LD & 178\\
Allan Griffiths & UKIP & 100\\
Richard Kail & Grn & 48\\
\end{tabular*}

\subsubsection*{Pound Hill South and Worth \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Pound Hill South and Worth , Crawley@Pound Hill S. \& Worth, \emph{Crawley}}

Resignation of Beryl Mecrow (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.57\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Alison Pendlington & C & 2435\\
Colin Flack & Lab & 1677\\
Daniel Elliott & Grn & 193\\
Harry Old & LD & 192\\
Janet Setford-Thompson & UKIP & 165\\
\end{tabular*}

\council{Mid Sussex}

\subsubsection*{Bolney \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Bolney , Mid Sussex@Bolney, \emph{Mid Sussex}}

Resignation of John Allen (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.58\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Judy Llewellyn-Burke & C & 562\\
Rodney Jackson & LD & 201\\
Peter Hopgood & UKIP & 59\\
Baron von Thunderclap & Loony & 33\\
\end{tabular*}

\subsubsection*{Burgess Hill Franklands \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Burgess Hill Franklands , Mid Sussex@Burgess Hill Franklands, \emph{Mid Sussex}}

Resignation of Chris King (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Claire Fussell & C & 757\\
Graham Allen & LD & 600\\
Elaine Bolton & Lab & 175\\
Chris French & UKIP & 91\\
Robert Duggan & Grn & 81\\
\end{tabular*}

\subsubsection*{Hassocks (2) \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June; 1 LD gain from C]}

\index{Hassocks , Mid Sussex@Hassocks, \emph{Mid Sussex}}

Resignations of Kevin Burke and Peter Martin (both C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Michelle Binks & C & 2023\\
Susan Hatton & LD & 1931\\
Jessica Edwards & C & 1835\\
Gemma Stockford & LD & 1536\\
Will Matthews & Lab & 1042\\
Harry Young & Lab & 812\\
Viv Aloy & Grn & 512\\
\end{tabular*}

\subsubsection*{East Grinstead Imberhorne \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[26th October]}

\index{East Grinsead Imberhorne , Mid Sussex@East Grinstead Imberhorne, \emph{Mid Sussex}}

Death of Bob Mainstone (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Rex Whittaker & C & 540\\
Tim Martin & LD & 206\\
David Wilbraham & Lab & 110\\
Ian Sanders & Ind & 67\\
\end{tabular*}

\council{Worthing}

\subsubsection*{Marine \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[3rd August; Lab gain from C]}

\index{Marine , Worthing@Marine, \emph{Worthing}}

Resignation of Joan Bradley (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Beccy Cooper & Lab & 1032\\
Joseph Crouch & C & 846\\
Antony Brown & LD & 246\\
Caroline Ponto & Grn & 55\\
\end{tabular*}

\section{Wiltshire}

\council{Wiltshire}

At the May 2017 ordinary election there was an unfilled vacancy in Trowbridge Lambrok division due to the resignation of Helen Osborn (Ind).
\index{Trowbridge Lambrok , Wiltshire@Trowbridge Lambrok, \emph{Wilts.}}

\section{Worcestershire}

\subsection*{County Council}\index{Worcestershire}

At the May 2017 ordinary election there was an unfilled vacancy in Redditch South division due to the resignation of Philip Gretton (C).
\index{Redditch South , Worcestershire@Redditch S., \emph{Worcs.}}

\council{Bromsgrove}

\subsubsection*{Norton \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[19th January]}

\index{Norton , Bromsgrove@Norton, \emph{Bromsgrove}}

Death of Pete Lammas (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Michael Webb & C & 219\\
Rory Shannon & Lab & 186\\
Adrian Smart & UKIP & 82\\
Michelle Baker & Grn & 20\\
\end{tabular*}

\subsubsection*{Alvechurch Village \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June; Ind gain from C]}

\index{Alvechurch Village , Bromsgrove@Alvechurch Village, \emph{Bromsgrove}}

Resignation of Roger Smith (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Kate van der Plank & Ind & 893\\
Luisa Nixon & C & 583\\
Siobhan Hughes & LD & 175\\
Kenneth Wheatley & Ind & 99\\
\end{tabular*}

\council{Malvern Hills}

\subsubsection*{West \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{West , Malvern Hills@West, \emph{Malvern Hills}}

Resignation of Julian Roskams (Grn).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Natalie McVey & Grn & 549\\
Henry Clarke & C & 437\\
Dee Tomlin & LD & 268\\
Andy Gardner & Lab & 169\\
\end{tabular*}

\council{Worcester}

\subsubsection*{Bedwardine \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Bedwardine , Worcester@Bedwardine, \emph{Worcester}}

Resignation of David Wilkinson (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Bill Amos & C & 1457\\
Jenny Barnes & Lab & 816\\
Daniel Daye & Grn & 189\\
Paul Hickling & UKIP & 124\\
\end{tabular*}

\council{Wychavon}

\subsubsection*{Droitwich South East \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Droitwich South East , Wychavon@Droitwich S.E., \emph{Wychavon}}

Resignation of Maureen Lawley (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Sital Harris & C & 1161\\
Adrian Key & LD & 258\\
Valerie Humphries & Lab & 216\\
Jane Zurek & Grn & 96\\
\end{tabular*}

\subsubsection*{Evesham South \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Evesham South , Wychavon@Evesham S., \emph{Wychavon}}

Resignation of Kenneth Barclay-Timmis (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Matthew Snape & C & 538\\
Julie Haines & LD & 187\\
Julie Tucker & Grn & 174\\
Emma Nishigaki & Ind & 154\\
\end{tabular*}

\council{Wyre Forest}

\sloppyword{ICHC = Independent Community and Health Concern}

\subsubsection*{Mitton \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[4th May]}

\index{Mitton , Wyre Forest@Mitton, \emph{Wyre Forest}}

Resignation of Sara Fearn (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Howard Williams & C & 797\\
John Thomas & ICHC & 506\\
Cliff Brewer & Lab & 429\\
John Holden & UKIP & 215\\
Simon Ford & LD & 79\\
Vicky Lea & Grn & 39\\
\end{tabular*}

\section{Glamorgan}

\council{Bridgend}

At the May 2017 ordinary election there were unfilled vacancies in Brackla and Cornelly divisions due to the resignations of David Sage (Lab) and Megan Butcher (Ind) respectively.
\index{Brackla , Bridgend@Brackla, \emph{Bridgend}}
\index{Cornelly , Bridgend@Cornelly, \emph{Bridgend}}

\council{Cardiff}

At the May 2017 ordinary election there were unfilled vacancies in Cyncoed, Llandaff North and Llanrumney divisions due to the resignations of Margaret Jones (LD) and Sue White (Lab) and the death of Derrick Morgan (Lab) respectively.
\index{Cyncoed , Cardiff@Cyncoed, \emph{Cardiff}}
\index{Llandaff North , Cardiff@Llandaff N., \emph{Cardiff}}
\index{Llanrumney , Cardiff@Llanrumney, \emph{Cardiff}}

\council{Merthyr Tydfil}

Merthyr = Merthyr Independents

\subsubsection*{Cyfarthfa (3) \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June; 1 Ind gain from Merthyr]}

\index{Cyfarthfa , Merthyr Tydfil@Cyfarthfa, \emph{Merthyr Tydfil}}

Ordinary election postponed from 4th May: death of candidate Ieuan Harris (Ind).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.5\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Geraint Thomas & Ind & 1369\\
Paul Brown & Merthyr & 1300\\
David Chaplin & Lab & 1202\\
Margaret Davies & Lab & 1137\\
Carol Estebanez & Lab & 1072\\
Terry Thomas & Merthyr & 1068\\
Bleddyn Hancock & Ind & 812\\
Mark Evans & PC & 685\\
\end{tabular*}

\council{Neath Port Talbot}

\subsubsection*{Bryn-côch South \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[23rd November]}

\index{Bryn-coch South , Neath Port Talbot@Bryn-côch S., \emph{Neath Port Talbot}}

Death of Janice Dudley (PC).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Jo Hale & PC & 525\\
Emma Denholm-Hall & Lab & 306\\
Peter Crocker-Jaques & C & 105\\
Sheila Kingston-Jones & LD & 92\\
Darren Thomas & UKIP & 33\\
\end{tabular*}

\section{Gwent}

\council{Caerphilly}

At the May 2017 ordinary election there was an unfilled vacancy in Aberbargoed division due to the death of Keith Reynolds (Lab).
\index{Aberbargoed , Caerphilly@Aberbargoed, \emph{Caerphilly}}

\council{Torfaen}

At the May 2017 ordinary election there was an unfilled vacancy in Trevethin division due to the disqualification (non-attendance) of John Marshall (Lab).
\index{Trevethin , Torfaen@Trevethin, \emph{Torfaen}}

\section{Mid and West Wales}

\council{Ceredigion}

\subsubsection*{Llandyfriog \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[8th June]}

\index{Llandyfriog , Ceredigion@Llandyfriog, \emph{Ceredigion}}

Ordinary election postponed from 4th May: death of candidate Neil Flower (LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
James Thomas & PC & \emph{unop.}\\
\end{tabular*}

\council{Powys}

At the May 2017 ordinary election there was an unfilled vacancy in Builth division due to the resignation of Avril York (Ind).
\index{Builth , Powys@Builth, \emph{Powys}}

\subsubsection*{Yscir \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[22nd June; C gain from Ind]}

\index{Yscir , Powys@Yscir, \emph{Powys}}

No candidates nominated in May 2017 ordinary election.

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Iain McIntosh & C & 165\\
Chris Davies & Ind & 144\\
Kate Heneghan & PC & 101\\
Bethan Irwin & Grn & 80\\
Daniel Evans & Ind & 62\\
Steve Davies & Ind & 2\\
\end{tabular*}

\section{North Wales}

\council{Denbighshire}

At the May 2017 ordinary election there was an unfilled vacancy in Denbigh Lower division due to the death of John Bartley (Ind).\index{Denbigh Lower , Denbighshire@Denbigh Lower, \emph{Denbighshire}}

\council{Flintshire}

At the May 2017 ordinary election there was an unfilled vacancy in Caerwys division due to the resignation of Jim Falshaw (C).
\index{Caerwys , Flintshire@Caerwys, \emph{Flintshire}}

\subsubsection*{Buckley Bistre West \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[9th November]}

\index{Buckley Bistre West , Flintshire@Buckley Bistre W., \emph{Flntshire}}

Death of Ron Hampson (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
Andy Williams & Lab & 398\\
Edith Hutchinson & Ind & 110\\
Martyn Teire & Ind & 86\\
Gren James & LD & 85\\
Louis Fox & C & 59\\
\end{tabular*}

\council{Isle of Anglesey}

At the May 2017 ordinary election there was an unfilled vacancy in Lligwy division due to the resignation of Derlwyn Hughes (Ind).
\index{Lligwy , Isle of Anglesey@Lligwy, \emph{Isle of Anglesey}}

\section[Aberdeenshire Councils]{\sloppyword{Aberdeenshire Councils}}

\council{Aberdeenshire}

At the May 2017 ordinary election there were unfilled vacancies in Aboyne, Upper Deeside and Donside; and Peterhead South and Cruden wards due to the resignations of Katrina Farquhar (C) and Tom Malone (Ind) respectively.
\index{Aboyne, Upper Deeside and Donside , Aberdeenshire@Aboyne, Upper Deeside \& Donside, \emph{Aberdeenshire}}
\index{Peterhead South and Cruden , Aberdeenshire@Peterhead S. \& Cruden, \emph{Aberdeenshire}}

\subsubsection*{Inverurie and District \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[12th October]}

\index{Inverurie and District , Aberdeenshire@Inverurie \& District, \emph{Aberdeenshire}}

Resignation of Colin Clark MP (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
\emph{First preferences}\\
Lesley Berry & C & 1672\\
Elaine Mitchell & SNP & 1146\\
Scott Bremner & LD & 295\\
Sarah Flavell & Lab & 276\\
Craig Stewart & Grn & 56\\
\end{tabular*}

\emph{Stewart eliminated:} Berry 1679, Mitchell 1161, Bremner 309, Flavell 290.

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
\emph{Flavell eliminated}\\
Lesley Berry & C & 1715\\
Elaine Mitchell & SNP & 1234\\
Scott Bremner & LD & 412\\
\end{tabular*}

\section{Ayrshire Councils}

\council{East Ayrshire}

Libtn = Libertarian Party

\subsubsection*{Kilmarnock East and Hurlford \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[26th January]}

\index{Kilmarnock East and Hurlford , East Ayrshire@Kilmarnock E. \& Hurlford, \emph{E. Ayrshire}}

Death of James Buchanan (SNP).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
\emph{First preferences}\\
Fiona Campbell & SNP & 1461\\
Dave Meechan & Lab & 881\\
Jon Herd & C & 602\\
Stephen McNamara & Libtn & 53\\
\end{tabular*}

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
\multicolumn{3}{@{\extracolsep{\fill}}l}{\emph{Herd and McNamara eliminated}}\\
%\emph{Herd and McNamara eliminated}\\
Fiona Campbell & SNP & 1531\\
Dave Meechan & Lab & 1122\\
\end{tabular*}

\section{Clyde Councils}

\council{Glasgow}

At the May 2017 ordinary election there was an unfilled vacancy in Canal ward due to the resignation of Chris Kelly (Lab).
\index{Canal , Glasgow@Canal, \emph{Glasgow}}

Libtn = Libertarian Party

\subsubsection*{Cardonald \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[7th September]}

\index{Cardonald , Glasgow@Cardonald, \emph{Glasgow}}

Death of Alistair Watson (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
\emph{First preferences}\\
Jim Kavanagh & Lab & 2614\\
Alex Mitchell & SNP & 1972\\
Thomas Haddow & C & 552\\
John Smith & Grn & 147\\
Isabel Nelson & LD & 80\\
Antony Sammeroff & Libtn & 12\\
\end{tabular*}

\emph{Nelson and Sammeroff eliminated:} Kavanagh 2641, Mitchell 1981, Haddow 566, Smith 161

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
\emph{Smith eliminated}\\
Jim Kavanagh & Lab & 2683\\
Alex Mitchell & SNP & 2066\\
Thomas Haddow & C & 574\\
\end{tabular*}

\council{North Lanarkshire}

BUSP = British Union and Sovereignty Party

\subsubsection*{Fortissat \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[7th September; Lab gain from C]}

\index{Fortissat , North Lanarkshire@Fortissat, \emph{N. Lanarks.}}

Resignation of Sandy Thornton (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
\emph{First preferences}\\
Clare Quigley & Lab & 1420\\
John Leckie & BUSP & 858\\
Mags Murphy & SNP & 761\\
Norma McNab & C & 424\\
Charlie Cefferty & Ind & 184\\
Kyle Davidson & Grn & 24\\
Daryl Gardner & UKIP & 18\\
\end{tabular*}

\emph{McNab, Cefferty, Davidson and Gardner eliminated}: Quigley 1546, Leckie 1088, Murphy 804

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
\emph{Murphy eliminated}\\
Clare Quigley & Lab & 1827\\
John Leckie & BUSP & 1139\\
\end{tabular*}

\council{Renfrewshire}

At the May 2017 ordinary election there was an unfilled vacancy in Houston, Crosslee and Linwood ward due to the resignation of Stuart Clark (Lab).
\index{Houston, Crosslee and Linwood , Renfrewshire@Houston, Crosslee \& Linwood, \emph{Renfs.}}

\council{South Lanarkshire}

\subsubsection*{Rutherglen Central and North \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[23rd November]}

\index{Rutherglen Central and North , South Lanarkshire@Rutherglen C. \& N., \emph{S. Lanarks.}}

Resignation of Ged Killen MP (Lab).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
\emph{First preferences}\\
Martin Lennon & Lab & 1173\\
David Innes & SNP & 836\\
Ellen Bryson & LD & 554\\
Taylor Muir & C & 368\\
Brian Finlay & Grn & 88\\
Janice Mackay & UKIP & 28\\
\end{tabular*}

\emph{Muir, Finlay and Mackay eliminated:} Lennon 1270, Innes 884, Bryson 711

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
\emph{Bryson eliminated}\\
Martin Lennon & Lab & 1541\\
David Innes & SNP & 989\\
\end{tabular*}

\section{Forth Councils}

\council{Fife}

At the May 2017 ordinary election there was an unfilled vacancy in Kirkcaldy East ward due to the death of Kay Carrington (Lab).
\index{Kirkcaldy East , Fife@Kirkcaldy E., \emph{Fife}}

\section{Highland Councils}

\council{Highland}

At the May 2017 ordinary election there was an unfilled vacancy in Wick ward due to the resignation of Gail Ross (SNP).
\index{Wick , Highland@Wick, \emph{Highland}}

Libtn = Libertarian Party

\subsubsection*{Tain and Easter Ross \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[28th September; Ind gain from LD]}

\index{Tain and Easter Ross , Highland@Tain \& Easter Ross, \emph{Highland}}

Resignation of Jamie Stone (LD).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
\emph{First preferences}\\
Alasdair Rhind & Ind & 1266\\
Stan Peace & SNP & 612\\
William Sinclair & LD & 372\\
Eva Short & C & 233\\
Gerard Holdsworth & Ind & 68\\
Harry Christian & Libtn & 13\\
\end{tabular*}

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
%\emph{Holdsworth and Christian eliminated:}\\
\multicolumn{3}{@{\extracolsep{\fill}}l}{\emph{Holdsworth and Christian eliminated}}\\
Alasdair Rhind & Ind & 1290\\
Stan Peace & SNP & 634\\
William Sinclair & LD & 387\\
Eva Short & C & 243\\
\end{tabular*}

\council{Moray}

\subsubsection*{Elgin City North \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[13th July; C gain from Ind]}

\index{Elgin City North , Moray@Elgin City N., \emph{Moray}}

Resignation of Sandy Cooper (Ind).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
\emph{First preferences}\\
Maria McLean & C & 923\\
Patsy Gowans & SNP & 895\\
Nick Taylor & Lab & 365\\
Terry Monaghan & Ind & 124\\
\end{tabular*}

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
\multicolumn{3}{@{\extracolsep{\fill}}l}{\emph{Taylor and Monaghan eliminated}}\\
%\emph{Taylor and Monaghan eliminated}\\
Maria McLean & C & 1061\\
Patsy Gowans & SNP & 994\\
\end{tabular*}

\section{Tay Councils}

\council{Perth and Kinross}

\subsubsection*{Perth City South \hspace*{\fill}\nolinebreak[1]%
\enspace\hspace*{\fill}
\finalhyphendemerits=0
[23rd November]}

\index{Perth City South , Perth and Kinross@Perth City S., \emph{Perth \& Kinross}}

Resignation of Michael Jamieson (C).

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
\emph{First preferences}\\
Pauline Leitch & SNP & 1780\\
Audrey Coates & C & 1734\\
Liz Barrett & LD & 1597\\
Tricia Duncan & Lab & 314\\
Elspeth Maclachlan & Grn & 102\\
Denise Baykal & Ind & 25\\
\end{tabular*}

\emph{Duncan, Maclachlan and Baykal eliminated:} Leitch 1883, Coates 1762, Barrett 1733

\noindent
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} p{0.53\columnwidth} >{\itshape}l r @{\extracolsep{\fill}}}
\emph{Barrett eliminated}\\
Audrey Coates & C & 2381\\
Pauline Leitch & SNP & 2227\\
\end{tabular*}

\end{resultsiii}
